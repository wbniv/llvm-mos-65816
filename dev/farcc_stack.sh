...
echo "==> opt-in control: WITHOUT +mos-farcc-stack the far ptr uses the DEFAULT (Imag32, the shipped winner); the flag is an opt-in override to variant (d)"
if "$TOOL/mos-clang" --target=mos -mcpu=mosw65816 "${A16[@]}" -Os -mllvm -verify-machineinstrs \
     -c -o /dev/null "$SRC" 2>"$NEGLOG"; then
  echo "  PASS: compiles by default (Imag32); +mos-farcc-stack overrides to variant (d) below"
else
