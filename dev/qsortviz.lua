-- dev/qsortviz.lua — MAME autoboot: let the qsort sort-visualizer demo run, snapshot the real PPU
-- output, and assert the gate proof (corpus_result) matches the host oracle. MAME's
-- video:snapshot() needs a real rendered surface, so dev/qsortviz.sh runs MAME under Xvfb. The
-- gate (qsortviz_gate_crc) finishes in a few frames; the bars re-sort continuously, so we snapshot
-- at SHOT_AT (default 700) to show the sorted bars. Inputs:
--   SHOT_ADDR  program-space addr of corpus_result (e.g. 0x7E00BE)
--   SHOT_WANT  expected hash (host reference, 0x8EA5)
...
local WANT = num("SHOT_WANT", 0x8EA5)
