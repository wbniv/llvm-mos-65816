-- dev/cgrade.lua — MAME autoboot: let the many-arg color-grade demo run, snapshot the real PPU
-- output, and assert the gate proof (corpus_result) matches the host oracle. MAME's
-- video:snapshot() needs a real rendered surface, so dev/cgrade.sh runs MAME under Xvfb. The
-- gate (cgrade_gate_crc) finishes in a few frames; the grade sweeps continuously, so we snapshot
-- at SHOT_AT (default 700) to show the graded scene. Inputs:
--   SHOT_ADDR  program-space addr of corpus_result (e.g. 0x7E00BE)
--   SHOT_WANT  expected hash (host reference, 0x783F)
...
local WANT = num("SHOT_WANT", 0x783F)
