-- dev/double-pendulum.lua — MAME autoboot: let the double-pendulum run, snapshot the real PPU
-- output, and assert corpus_result matches the host oracle. Inputs via environment:
--   SHOT_ADDR  program-space address of corpus_result (e.g. 0x7E1234)
--   SHOT_WANT  expected gate hash (host oracle value)
--   SHOT_AT    frame to snapshot/assert at (default 500)
local function num(name, d)
  local v = os.getenv(name)
  if not v or v == "" then return d end
  return tonumber(v) or tonumber(v, 16) or d
end
