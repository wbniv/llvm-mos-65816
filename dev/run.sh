...
Targets:
  ...
  far_fnptr  #320 follow-up (a): far FUNCTION POINTERS — an indirect call through a
             24-bit code pointer; build examples/65816/far_fnptr.c (snes-far, +mos-a16),
             assert the call is JSL __call_indir_far, the target carries a #mos24bank
             reloc, far_leaf (bank $01) RTLs, boot in MAME, check far_leaf(0x5A) == 0xFF
  far_fnptr_var #320 follow-up (a): far FUNCTION POINTERS, typed-VARIABLE surface —
             a far code pointer STORED in a variable (far_fn_t fp = far_leaf; fp(x)),
             the `far` attribute riding the typedef; same backend mechanism + gate as
             far_fnptr; build examples/65816/far_fnptr_var.c, boot in MAME, == 0xFF
  far_sizeof #320 follow-up (a): sizeof(far*) == 4 (getPointerWidthV(AS2)->32) — the
             _Static_assert(sizeof(FAR*)==4) gates the size at compile time; a far
             pointer stored in a struct field is deref'd, asserting the 4-byte store
             didn't clobber the adjacent field; build far_sizeof.c, MAME, == 0xD1
