-- dev/truchet.lua — MAME autoboot: let the 64-bit Avalanche explorer run, snapshot the real PPU output, and
-- assert the gate proof (corpus_result) matches the host oracle. MAME's video:snapshot() needs a
-- real rendered surface, so dev/truchet.sh runs MAME under Xvfb. The gate (truchet_gate_crc) finishes
...
--   SHOT_WANT  expected hash (host reference, 0xB3E6)
...
local WANT = num("SHOT_WANT", 0xB3E6)
