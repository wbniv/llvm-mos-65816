-- dev/boids.lua — MAME autoboot: let the Boids flock run, snapshot the real PPU output, and assert the
-- gate proof (corpus_result) matches the host oracle. MAME's video:snapshot() needs a real rendered
-- surface, so dev/boids.sh runs MAME under Xvfb. The gate (boids_gate_crc) is computed behind the title
-- card during boot; we snapshot at SHOT_AT (default 1400, past the title fly-in + hold) to show the
-- swirling flock. Inputs:
--   SHOT_ADDR  program-space addr of corpus_result (e.g. 0x7E____)
--   SHOT_WANT  expected hash (host reference, 0xA8AB)
...
local WANT = num("SHOT_WANT", 0xA8AB)
