-- dev/gf256.lua — MAME autoboot: let the GF(2^8) Galois field demo run, snapshot the real PPU
-- output, and assert the gate proof (corpus_result) matches the host oracle. MAME's
-- video:snapshot() needs a real rendered surface, so dev/gf256.sh runs MAME under Xvfb. The
-- gate (gf256_gate_crc) finishes in a few frames; the scope scrolls continuously, so we snapshot
