-- dev/sodo.lua — MAME autoboot: let the signed-64 odometer demo run, snapshot the real PPU
-- output, and assert the gate proof (corpus_result) matches the host oracle. MAME's
-- video:snapshot() needs a real rendered surface, so dev/sodo.sh runs MAME under Xvfb. The
-- gate (sodo_gate_crc) finishes in a few frames; the odometer tape scrolls, so we snapshot
-- at SHOT_AT (default 700) to show the tape. Inputs:
--   SHOT_ADDR  program-space addr of corpus_result (e.g. 0x7E00BE)
--   SHOT_WANT  expected hash (host reference, 0xD2A2)
...
local WANT = num("SHOT_WANT", 0xD2A2)
