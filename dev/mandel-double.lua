-- dev/mandel-double.lua — MAME autoboot: let the double-precision soft-float Mandelbrot run, snapshot the real PPU
-- output, and assert the gate proof (corpus_result) matches the host oracle. MAME's video:snapshot()
-- needs a real rendered surface, so dev/mandel-double.sh runs MAME under Xvfb. The gate (mf_gate_crc)
...
--   SHOT_WANT  expected hash (host reference, 0x0EDF)
...
local WANT = num("SHOT_WANT", 0x0EDF)
