-- dev/lzdec.lua — MAME autoboot: let the LZ-decode image-reveal demo run, snapshot the real PPU
-- output, and assert the gate proof (corpus_result) matches the host oracle. MAME's
-- video:snapshot() needs a real rendered surface, so dev/lzdec.sh runs MAME under Xvfb. The
-- gate (lzdec_gate_crc) finishes in a few frames; the image reveals progressively, so we snapshot
-- at SHOT_AT (default 700) to show the decoded image. Inputs:
--   SHOT_ADDR  program-space addr of corpus_result (e.g. 0x7E00BE)
--   SHOT_WANT  expected hash (host reference, 0x0100)
...
local WANT = num("SHOT_WANT", 0x0100)
