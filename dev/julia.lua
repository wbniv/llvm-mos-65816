-- dev/julia.lua — MAME autoboot: let the Julia explorer run, snapshot the real PPU output, and
-- assert the gate proof (corpus_result) matches the host oracle. MAME's video:snapshot() needs a
-- real rendered surface, so dev/julia.sh runs MAME under Xvfb. The gate (julia_gate_crc) finishes
...
--   SHOT_WANT  expected hash (host reference, 0x3490)
...
local WANT = num("SHOT_WANT", 0x3490)
