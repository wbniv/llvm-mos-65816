-- dev/critters.lua — MAME autoboot: let the protothread critter swarm demo run, snapshot the real PPU
-- output, and assert the gate proof (corpus_result) matches the host oracle. MAME's
-- video:snapshot() needs a real rendered surface, so dev/critters.sh runs MAME under Xvfb. The
-- gate (critters_gate_crc) finishes in a few frames; the critters patrol continuously, so we snapshot
-- at SHOT_AT (default 700) to show the swarm. Inputs:
--   SHOT_ADDR  program-space addr of corpus_result (e.g. 0x7E00BE)
--   SHOT_WANT  expected hash (host reference, 0xAD9F)
...
local WANT = num("SHOT_WANT", 0xAD9F)
