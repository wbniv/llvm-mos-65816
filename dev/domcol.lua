-- dev/domcol.lua — MAME autoboot: let the domain-colouring demo run, snapshot the real PPU
-- output, and assert the gate proof (corpus_result) matches the host oracle. MAME's
-- video:snapshot() needs a real rendered surface, so dev/domcol.sh runs MAME under Xvfb. The
-- gate (domcol_gate_crc) finishes in a few frames; the scope scrolls continuously, so we snapshot
