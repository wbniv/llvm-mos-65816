-- dev/pi.lua — MAME autoboot: let the π spigot run, snapshot the real PPU output, and
-- assert the gate proof (corpus_result) matches the host oracle. MAME's video:snapshot() needs a
-- real rendered surface, so dev/pi.sh runs MAME under Xvfb. The gate (pi_gate_crc) finishes
-- in ~40 frames (0.67 s emulated) and the scatter canvas fills over ~3600; we snapshot at
-- SHOT_AT (default 500) to show digits + scatter both started. Inputs:
--   SHOT_ADDR  program-space addr of corpus_result (e.g. 0x7E137A)
--   SHOT_WANT  expected hash (host reference, 0x97F3)
...
local WANT = num("SHOT_WANT", 0x97F3)
