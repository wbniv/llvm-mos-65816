-- dev/funnelkal.lua: MAME assert for Funnel-Shift Kaleidoscope (#73).
-- Reads corpus_result from WRAM at the address supplied by SHOT_ADDR,
-- compares with SHOT_WANT, snapshots the screen, then exits.
local function num(k, d) local v = os.getenv(k); return v and tonumber(v) or d end
local ADDR = num("SHOT_ADDR", 0x7E0000)
local WANT = num("SHOT_WANT", 0xEED4)
