-- dev/huffman.lua — MAME autoboot: let the Huffman decode demo run, snapshot the real PPU
-- output, and assert the gate proof (corpus_result) matches the host oracle. MAME's
-- video:snapshot() needs a real rendered surface, so dev/huffman.sh runs MAME under Xvfb. The
-- gate (huffman_gate_crc) finishes in a few frames; the scope scrolls continuously, so we snapshot
