-- dev/epicycles.lua — MAME autoboot: let the Fourier-epicycles demo run, snapshot the real PPU
-- output, and assert the gate proof (corpus_result) matches the host oracle. MAME's
-- video:snapshot() needs a real rendered surface, so dev/epicycles.sh runs MAME under Xvfb. The
-- gate (epi_gate_crc) finishes well under 100 frames; we snapshot at SHOT_AT (default 500) to show
-- the full star outline drawn over its generating circles. Inputs:
--   SHOT_ADDR  program-space addr of corpus_result (e.g. 0x7E1366)
--   SHOT_WANT  expected hash (host reference, 0x4F6C)
...
local WANT = num("SHOT_WANT", 0x4F6C)
