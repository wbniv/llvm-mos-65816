-- dev/radix.lua — MAME autoboot: let the radix sort demo run, snapshot the real PPU
-- output, and assert the gate proof (corpus_result) matches the host oracle. MAME's
-- video:snapshot() needs a real rendered surface, so dev/radix.sh runs MAME under Xvfb. The
-- gate (radix_gate_crc) finishes in a few frames; the scope scrolls continuously, so we snapshot
