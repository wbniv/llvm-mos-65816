-- dev/hdr-bloom.lua — MAME autoboot: let the HDR additive-bloom demo run, snapshot the real PPU
-- output, and assert the gate proof (corpus_result) matches the host oracle. MAME's
-- video:snapshot() needs a real rendered surface, so dev/hdr-bloom.sh runs MAME under Xvfb. The
-- gate (hdr_bloom_gate_crc) finishes in a few frames; the bloom drifts continuously, so we snapshot
-- at SHOT_AT (default 500) to show the field lit. Inputs:
--   SHOT_ADDR  program-space addr of corpus_result (e.g. 0x7E0050)
--   SHOT_WANT  expected hash (host reference, 0xF951)
...
local WANT = num("SHOT_WANT", 0xF951)
