-- dev/raycaster.lua — MAME autoboot: let the Raycaster run, snapshot the real PPU output, and
-- assert the gate proof (corpus_result) matches the host oracle. The gate (rc_gate_crc) is computed
-- at startup (64-column fan, fast); we snapshot at SHOT_AT (default 400) to show the rendered maze view.
-- Inputs:
--   SHOT_ADDR  program-space addr of corpus_result (e.g. 0x7E0200)
--   SHOT_WANT  expected hash from host oracle (e.g. 0xB200)
