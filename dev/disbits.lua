-- dev/disbits.lua — MAME autoboot: let the cross-byte bitfield disassembler demo run, snapshot the real PPU
-- output, and assert the gate proof (corpus_result) matches the host oracle. MAME's
-- video:snapshot() needs a real rendered surface, so dev/disbits.sh runs MAME under Xvfb. The
-- gate (disbits_gate_crc) finishes in a few frames; the disassembly scrolls, so we snapshot
-- at SHOT_AT (default 700) to show the decoded field map. Inputs:
--   SHOT_ADDR  program-space addr of corpus_result (e.g. 0x7E00BE)
--   SHOT_WANT  expected hash (host reference, 0x31D7)
...
local WANT = num("SHOT_WANT", 0x31D7)
