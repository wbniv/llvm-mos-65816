-- dev/bitcensus.lua — MAME autoboot: let the Bit-Census Field demo run, snapshot the real PPU output,
-- and assert the gate proof (corpus_result) matches the host oracle. MAME's video:snapshot() needs a
-- real rendered surface, so dev/bitcensus.sh runs MAME under Xvfb. The gate (bitcensus_gate_crc)
-- finishes during the title card; the field scrolls + cycles intrinsics continuously, so we snapshot
-- at SHOT_AT (default 500) to show the texture. Inputs:
--   SHOT_ADDR  program-space addr of corpus_result (e.g. 0x7E00BE)
--   SHOT_WANT  expected hash (host reference, 0x9516)
...
local WANT = num("SHOT_WANT", 0x9516)
