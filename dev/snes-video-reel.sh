...
presented_vma=$(awk '$NF=="video_reel_presented" {print $1; exit}' "$MAP")
...
case "$CADENCE" in
  3) default_presented=142 ;;
  2) default_presented=1e2 ;;
  1) default_presented=3c5 ;;
...
if [ -f "$TILES" ] && [ -f "$PALETTE" ]; then
  python3 "$ROOT/tools/snes-video-screenshot-check.py" --frame 1 \
