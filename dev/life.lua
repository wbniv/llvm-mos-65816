-- dev/life.lua — MAME autoboot: let Conway's Game of Life run, snapshot the real PPU output,
-- and assert the gate proof (corpus_result) matches the host oracle. The gate (life_gate_crc)
-- is computed at startup (~11 frames, 32 gens of a 64×48 grid); we snapshot at SHOT_AT (default
-- 400) to show the glider gun firing into the settled soup. Inputs:
--   SHOT_ADDR  program-space addr of corpus_result (e.g. 0x7E0200)
--   SHOT_WANT  expected hash from host oracle (e.g. 0xDDF1)
