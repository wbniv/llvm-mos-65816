#!/usr/bin/env bash
# dev/farcc_axy.sh — #320 Inc 4 Phase 2 (A2): far-pointer CC variant (c) "A:X+Y"
# (low-16 offset in A:X — A=low, X=high — + bank byte in Y). Builds the SHARED
# variant-agnostic round-trip source examples/65816/farcc_imag32.c (a far pointer
# returned from make_far_ptr() AND passed into deref_far() across noinline calls)
# with -mcpu=mosw65816 +mos-a16 +mos-farcc-axy, and asserts the 24-bit address
# round-trips to 0xF3 on both emulators — proving the 3-way {A,X,Y} hardware-register
# split (assignCustomValue) (de)composes the pointer correctly in both directions.
#
...
ROM="$BUILD/farcc_axy.sfc"
MAP="$BUILD/farcc_axy.map"
OBJ="$BUILD/farcc_axy.o"
NEGLOG="$BUILD/farcc_axy_noflag.err"
A16=(-Xclang -target-feature -Xclang +mos-a16)
FARCC=(-Xclang -target-feature -Xclang +mos-farcc-axy)
WANT=0xF3
...
echo "==> execution gate: boot in MAME, assert corpus_result == $WANT (far ptr round-tripped via A:X+Y split)"
source "$ROOT/dev/_emu.sh"
require_bios || exit $?
if run_assert "$ROM" "$MAP" corpus_result "$WANT"; then
  emu_verdict 0 "far pointer carried in A:X (offset) + Y (bank) across real calls (variant c); 24-bit address survived both directions, byte read == $WANT (bsnes-jg confirms via dev/run.sh xcheck)"
else
  emu_verdict 1 ""
  exit 1
fi
