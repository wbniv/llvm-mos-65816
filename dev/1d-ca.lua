-- dev/1d-ca.lua — MAME autoboot: let the Rule 90/110 CA run, snapshot the real PPU output,
-- and assert the gate proof (corpus_result) matches the host oracle. The gate (ca_gate_crc)
-- finishes in ~10 frames (256 CA steps); we snapshot at SHOT_AT (default 400) to show
-- several hundred rows of the scrolling pattern. Inputs:
--   SHOT_ADDR  program-space addr of corpus_result (e.g. 0x7E0200)
--   SHOT_WANT  expected hash from host oracle (e.g. 0xABCD)
