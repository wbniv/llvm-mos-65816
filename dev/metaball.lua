-- dev/metaball.lua — MAME autoboot: let the union-punned metaball demo run, snapshot the real PPU
-- output, and assert the gate proof (corpus_result) matches the host oracle. MAME's
-- video:snapshot() needs a real rendered surface, so dev/metaball.sh runs MAME under Xvfb. The
-- gate (metaball_gate_crc) finishes in a few frames; the metaball field animates, so we snapshot
-- at SHOT_AT (default 700) to show the blobs. Inputs:
--   SHOT_ADDR  program-space addr of corpus_result (e.g. 0x7E00BE)
--   SHOT_WANT  expected hash (host reference, 0xAEBE)
...
local WANT = num("SHOT_WANT", 0xAEBE)
