...
printf '%s\n' "$DIS" | grep -iE '^\s*[0-9a-f]+:\s*(18|c2 20|e2 20|65|a5|85)\b' | head
nrep=$(printf '%s\n' "$DIS" | grep -ciE '^\s*[0-9a-f]+:\s*c2 20\b')
nsep=$(printf '%s\n' "$DIS" | grep -ciE '^\s*[0-9a-f]+:\s*e2 20\b')
nadc=$(printf '%s\n' "$DIS" | grep -ciE '^\s*[0-9a-f]+:\s*65\b')   # 65 = adc zp (native Imag16)
...
[ "$nadc" -ge 1 ] && echo "  PASS: 16-bit adc zp present (native Imag16 path)" || { echo "  FAIL: no adc zp"; rc=1; }
