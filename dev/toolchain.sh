...
usage() {
  ...
  echo "  also refreshes the lit tool set (llc/opt/llvm-mc/llvm-objdump/llvm-readobj/llvm-readelf/llvm-config/count/split-file/FileCheck/not)"
...
# The distribution installs clang+lld; lit also needs the codegen and MC tools,
# RUN-line helpers, and llvm-config. Build llvm-readelf explicitly: it is a
# separate alias target for llvm-readobj. llvm-lit is generated at configure
# time and has no build target. Keep this set in sync with dev/lit.sh.
