-- dev/grid3d.lua — MAME autoboot: let the 3-D grid voxel life demo run, snapshot the real PPU
-- output, and assert the gate proof (corpus_result) matches the host oracle. MAME's
-- video:snapshot() needs a real rendered surface, so dev/grid3d.sh runs MAME under Xvfb. The
-- gate (grid3d_gate_crc) finishes in a few frames; the scope scrolls continuously, so we snapshot
-- at SHOT_AT (default 700) to show the scope trace. Inputs:
--   SHOT_ADDR  program-space addr of corpus_result (e.g. 0x7E00BE)
--   SHOT_WANT  expected hash (host reference, 0xFCDE)
...
local WANT = num("SHOT_WANT", 0xFCDE)
local AT   = num("SHOT_AT", 800)
