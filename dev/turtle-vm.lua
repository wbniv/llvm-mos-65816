-- dev/turtle-vm.lua — MAME autoboot: let the Bytecode-VM Turtle run, snapshot the real PPU output, and
-- assert the gate proof (corpus_result) matches the host oracle. MAME's video:snapshot() needs a
-- real rendered surface, so dev/turtle-vm.sh runs MAME under Xvfb. The gate (turtle-vm_gate_crc) finishes
...
--   SHOT_WANT  expected hash (host reference, 0x4007)
--   SHOT_AT    frame to snapshot/assert at (default 600)
...
local WANT = num("SHOT_WANT", 0x4007)
