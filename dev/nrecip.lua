-- dev/nrecip.lua — MAME autoboot: let the Newton-reciprocal floor demo run, snapshot the real PPU
-- output, and assert the gate proof (corpus_result) matches the host oracle. MAME's
-- video:snapshot() needs a real rendered surface, so dev/nrecip.sh runs MAME under Xvfb. The
-- gate (nrecip_gate_crc) finishes in a few frames; the floor scrolls forward, so we snapshot
-- at SHOT_AT (default 700) to show the floor. Inputs:
--   SHOT_ADDR  program-space addr of corpus_result (e.g. 0x7E00BE)
--   SHOT_WANT  expected hash (host reference, 0x044A)
...
local WANT = num("SHOT_WANT", 0x044A)
