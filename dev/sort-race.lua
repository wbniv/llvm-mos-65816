-- dev/sort-race.lua — MAME autoboot: let the Sorting Race demo run, snapshot the real PPU output,
-- and assert the gate proof (corpus_result) matches the host oracle. MAME's video:snapshot() needs
-- a real rendered surface, so dev/sort-race.sh runs MAME under Xvfb. corpus_result is set once at
-- startup (sortrace_gate_crc, ~tens of frames), after which the three bands animate; we snapshot at
-- SHOT_AT (default 500) to show the bars mid-race. Inputs:
--   SHOT_ADDR  program-space addr of corpus_result (e.g. 0x7E137A)
--   SHOT_WANT  expected hash (host reference, 0xB28F)
...
local WANT = num("SHOT_WANT", 0xB28F)
