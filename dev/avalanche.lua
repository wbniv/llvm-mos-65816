-- dev/avalanche.lua — MAME autoboot: let the 64-bit Avalanche explorer run, snapshot the real PPU output, and
-- assert the gate proof (corpus_result) matches the host oracle. MAME's video:snapshot() needs a
-- real rendered surface, so dev/avalanche.sh runs MAME under Xvfb. The gate (avalanche_gate_crc) finishes
...
--   SHOT_WANT  expected hash (host reference, 0x27EA)
...
local WANT = num("SHOT_WANT", 0x27EA)
