-- dev/duff.lua — MAME autoboot: let the Duff-device dissolve transition demo run, snapshot the real PPU
-- output, and assert the gate proof (corpus_result) matches the host oracle. MAME's
-- video:snapshot() needs a real rendered surface, so dev/duff.sh runs MAME under Xvfb. The
-- gate (duff_gate_crc) finishes in a few frames; the dissolve runs continuously, so we snapshot
-- at SHOT_AT (default 700) to show the dissolve. Inputs:
--   SHOT_ADDR  program-space addr of corpus_result (e.g. 0x7E00BE)
--   SHOT_WANT  expected hash (host reference, 0x5531)
...
local WANT = num("SHOT_WANT", 0x5531)
