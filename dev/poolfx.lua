-- dev/poolfx.lua — MAME autoboot: let the free-list particle-fountain demo run, snapshot the real PPU
-- output, and assert the gate proof (corpus_result) matches the host oracle. MAME's
-- video:snapshot() needs a real rendered surface, so dev/poolfx.sh runs MAME under Xvfb. The
-- gate (poolfx_gate_crc) finishes in a few frames; the fountain sprays continuously, so we snapshot
-- at SHOT_AT (default 500) to show the spray. Inputs:
--   SHOT_ADDR  program-space addr of corpus_result (e.g. 0x7E00BE)
--   SHOT_WANT  expected hash (host reference, 0x2B9B)
...
local WANT = num("SHOT_WANT", 0x2B9B)
