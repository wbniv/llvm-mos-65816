-- dev/factorial.lua — MAME autoboot: let the bignum factorial demo run, snapshot the real PPU
-- output, and assert the gate proof (corpus_result) matches the host oracle. MAME's
-- video:snapshot() needs a real rendered surface, so dev/factorial.sh runs MAME under Xvfb.
-- The gate CRC (factorial_gate_crc, 50!) finishes in ~15 emulated frames; we snapshot at SHOT_AT
-- (default 500) to show several factorial steps rendered on screen. Inputs:
--   SHOT_ADDR  program-space addr of corpus_result (e.g. 0x7E0100)
--   SHOT_WANT  expected hash (host reference, 0x772F for 50!)
...
local WANT = num("SHOT_WANT", 0x772F)
