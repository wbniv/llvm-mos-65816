-- dev/harmonograph.lua — MAME autoboot: let the Harmonograph run, snapshot the real PPU output, and
-- assert the gate proof (corpus_result) matches the host oracle. The gate (harmo_gate_crc) is computed
-- at startup (256 samples, fast); we snapshot at SHOT_AT (default 400) to show a bloomed figure.
-- Inputs:
--   SHOT_ADDR  program-space addr of corpus_result (e.g. 0x7E0200)
--   SHOT_WANT  expected hash from host oracle (e.g. 0x0EBB)
