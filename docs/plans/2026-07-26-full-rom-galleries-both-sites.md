...
| **`0012`** `LDCImm` set lowering | Baseline `MOSMCInstLower` handled only `0`→`CLC` and `-1`→`SEC`, although an i1 immediate can be plain `1`; that hit `llvm_unreachable` in assertions builds and UB under NDEBUG. | Lower any nonzero i1 as `SEC`. | [`a320cbd`](https://github.com/wbniv/llvm-mos-65816/commit/a320cbd) | No ROM claimed; `borrowlad` remains native-width subtraction coverage only. | Direct `mos65c02` MIR regression in `asm-printer.mir`; see [`docs/upstream-ldcimm-set-lowering-pr.md`](../upstream-ldcimm-set-lowering-pr.md#reproduction). |
