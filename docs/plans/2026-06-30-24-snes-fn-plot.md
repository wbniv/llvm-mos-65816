...
The demo parses four baked expressions (no user input) and plots each as a connected pixel curve
on a 128×128 BitmapCanvas (consecutive columns joined by `canvas_line()` to avoid gaps on steep
sections), cycling automatically with an animated TitleLayer intro card. The gate evaluates
`fn_exprs[0]` ("x\*x-0.5") at 64 x-values in [-2,2] and CRCs the IEEE float bit-patterns.
...
Evaluation (PIXELS_PER_FRAME=2 columns per frame):
  ...
  if prev_py in [0,127] and py in [0,127]:
    canvas_line(px-1, prev_py, px, py, 1)   -- connects columns, fills steep gaps
  else:
    canvas_plot(px, py, 1)                   -- first column or clipped
...
| `examples/snes/fn-plot.c` | SNES ROM: BitmapCanvas + TextLayer + TitleLayer; `canvas_line` connected curves at 2px/frame |
...
| Asset | From | Used for |
|-------|------|---------|
| `snesgfx/bitmap_canvas.h` | spirograph / n-body | 128×128 2bpp pixel surface; `canvas_line` for connected curves |
| `snesgfx/text_layer.h` | spirograph / n-body | 2-row HUD (expression + progress) |
| `snesgfx/title_layer.h` | all demos | animated intro card ("FN-PLOT / RECURSIVE PARSER") |
...
Two-stage publish (per updated skill sequencing):

**Stage A** — default-8-bit published immediately after initial gate PASS.
**Stage B** — +mos-a16 ROM published after full 5-way verification (overwrites Stage A).

Final published ROM: `+mos-a16` build at `corpus_result @ WRAM 0x002a`.
...
/snes-rom-page
  ...
  --selfcheck "0x002a 2 0x2EBE 500 fn-plot"
```

Published: [biohack.net/snes/fn-plot/](https://biohack.net/snes/fn-plot/) — biohack.net v1.0.138 (initial), v1.0.139 (fixed: TitleLayer + canvas_line + 2px/frame).
...
2. ROM builds clean; `snes-checksum.py` exits 0.
   ...
   ```
   built build/fn-plot.sfc (+mos-a16); corpus_result @ WRAM 0x002a
   ```
   PASS (after fix — original build had 0x135d before TitleLayer added)

3. Corpus slice host-compiles; compile exit 0. PASS

4. `dev/run.sh fn-plot` — host oracle + disasm gate + bsnes-jg + MAME PASS.
   ...
   SMOKE: PASS off=0x2A len=2 got=0x2EBE (ran 500 frames, bsnes-jg)
...
6. **Title intro card** — bsnes-jg frame-500 render shows the curve drawing animation with HUD (title card faded before frame 500). PASS

7. **Plan title card** — `build/fn-plot-jg.png` copied to `docs/plans/screenshots/fn-plot.png`, updated after fix. PASS

8. `/snes-rom-page` re-published v1.0.139 with fixed ROM + updated selfcheck offset.

## Post-ship fixes (2026-06-30)

Initial publish (v1.0.138) was missing:
- **TitleLayer intro card** — screen was blank during gate CRC startup
- **`canvas_line` between columns** — steep curves had visible pixel gaps (isolated dots)
- **Speed** — `PIXELS_PER_FRAME` was 1 (too slow, 128 frames per curve); raised to 2
- **Buffer overflow warning** — `char buf[7]` used for 20-char progress bar; fixed to `buf[21]`

Fixed in commit `d65c75e`; re-published as biohack.net v1.0.139. Gate CRC unchanged (`0x2EBE`); `corpus_result` WRAM address moved from `0x135d` → `0x002a` due to TitleLayer adding BSS.
