...
| SPC700 | `-O2` on any function that loads an immediate into an imaginary register crashes in `MOSLateOptimization::combineLdImm` on the unpatched `llc-final` too (null `Load`); the SPC700 test avoids the shape; tracked in `TODO.md` (T2) |
