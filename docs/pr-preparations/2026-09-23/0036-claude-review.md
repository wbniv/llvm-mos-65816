...
in two bytes instead of three. Out-of-bounds accesses are undefined in C and
already broken under `abs,x` at page 0xFF anyway. The assembler makes the same
...
| c-torture, 1,390 files × `-O0/-O2/-Os`, before → after 0036 | 4,170 comparisons: 0 new failures, 0 assembly changes (no zero-page sections in that corpus, and zero-page-allocated stores already use `STZpIdx`) |
