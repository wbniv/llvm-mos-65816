...
- ~~**MC-layer `cop` mnemonic (assembler gap, found by demo #140, 2026-08-04).**~~ ✅ **POSTED
  ...
  (`$02` is `NXT` on the 65EL02, pinned by a negative RUN line). MC suite fully green — 40/40 on the PR branch. **Correction comment POSTED 2026-08-05** ([#588 comment](https://github.com/llvm-mos/llvm-mos/pull/588#issuecomment-5193238041)) retracting the body's stale "39/40 / addr-asciz pre-existing" claim (stale-`llvm-readelf` artifact; suite fully green). [CORRECTED 2026-08-04: the earlier '5 pre-existing failures on pristine tip' / '39/40 lone failure' claims were exit-127 tool-missing artifacts of the minimal build-pr tool set (opt, llvm-readelf absent); with the tools built the suites are fully green — CodeGen 79 pass + getchar-regression.ll upstream-disabled (UNSUPPORTED: target), 0 failures; MC 39/39 (+ the branch's own new tests). Rule: build the tools the suite RUNs before quoting numbers; exit-127 in a lit log is an environment defect.] Body pre-flighted: source link verified 200 post-push; a16/xy16 wording de-forked
