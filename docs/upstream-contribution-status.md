...
| 11 | **LTO + `+mos-a16` bitmask-loop early exit** — 32-bit shift-by-r loop exits at r = 16 | **issue** | LTO miscompile (silent wrong output, no crash, no verifier trip): a loop over `(uint32_t)1u<<r` for r = 0–27 generates `cmp r, #16; bcc normal_path; jmp rts` — the function returns at the first r ≥ 16 iteration, skipping all high bits. Confirmed in factorial-demo `_fact_emit` (pre-fix ROM `build/factorial-debug.sfc.elf`). No compiler fix yet (workaround: avoid the 32-bit zero-compare sentinel). Isolated to LTO: `-fno-lto` generates correct code. | [issue body](321-upstream-lto-a16-bitmask-loop-early-exit-issue.md) | n/a (issue; no fix patch yet) |
