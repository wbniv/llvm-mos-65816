...
**Last updated:** 2026-07-31 (**NEW READY-TO-POST ARTIFACT — item 15, `mos-late-opt` non-GPR `LDImm`
destination.** A null-pointer store in `MOSLateOptimization::combineLdImm` crashes the backend on any
`LDImm` whose destination is not `A`/`X`/`Y` — legal on SPC700, where `MOSInstrInfo::getRegClass`
widens the def class to `Anyi8` for `mov dp, #imm`. **Reproduces on pristine upstream**, from 7 lines
of ordinary C at `-mcpu=mosspc700 -Os`; no fork feature involved. Fix + `late-opt-spc700.mir`
regression carried as fork patch `0003` (reusing the retired `0003-late-opt-txy-dead-flag`/PR #562
slot and lifecycle), `patch -p1 --dry-run` clean against pristine `8be054612`. Red/green proven by an
A/B rebuild of `llc`. **POSTED 2026-07-31 as
[PR #584](https://github.com/llvm-mos/llvm-mos/pull/584)** — branch `wbniv:mos-late-opt-nongpr-ldimm`
@ `ae5c399eb277`, minted from upstream tip `8b616af9434f` (patch re-verified `apply --check` clean
there). Found while root-causing #138, whose LZSS gallery reached the same
null store via a 32-bit imaginary far-pointer destination. Artifact finalized same day: the PR body's
real-world-sighting section links the live demo
([biohack.net/snes/lzss-gallery](https://biohack.net/snes/lzss-gallery/), canonical 200 URL), and the
staged post runbook carries the two post-posting steps — flip this item to posted with the PR number,
and refresh wald3n.com's public contributions snapshot via `task open-source:refresh` (+ commit/deploy)
so its staleness gate stays green.) Previously 2026-07-26 #5 (**🏁 CAMPAIGN WAVE 1 COMPLETE — all three items posted.** Item 2:
...
| 15 | **`mos-late-opt` non-GPR `LDImm` destination** — null-pointer crash on SPC700 | **fix PR** | Upstream crash in release *and* asserts builds alike — it is a null *store*, not an assert: `MOSLateOptimization::combineLdImm` selects one of three `ImmLoad` trackers with a `switch` over `{A, X, Y}` and then stores through the result unconditionally, so any `LDImm` with a different destination writes to address `0`. On SPC700 that destination is legal — `MOSInstrInfo::getRegClass` deliberately widens `LDImm`'s def class to `Anyi8` so the pseudo can model `mov dp, #imm` — so `$rcN = LDImm imm` is verifier-clean MIR the allocator emits from ordinary C. **Reproduces on pristine upstream** (`build/upstream-llc`): no fork feature, no `+mos-a16`, no far pointers, no LTO — 7 lines of C at `-mcpu=mosspc700 -O1/-O2/-Os/-Oz` (clean at `-O0`), or 4 lines of MIR. **Fix** = `if (!MOS::GPRRegClass.contains(Dst)) continue;`; skipping is the correct semantic rather than merely the safe one — an imaginary destination writes no GPR, so the tracked A/X/Y values stay valid across it and must *not* be invalidated. Regression `llvm/test/CodeGen/MOS/late-opt-spc700.mir` (minimal crasher + transparency of an imaginary destination to the `LDX #imm`→`TAX` rewrite). Red/green proven by an A/B rebuild: **8 lit failures with the guard reverted (including the new test), 7 with it** — the other 7 pre-existing and identical across both runs. Found via the #138 LZSS gallery, where a 32-bit imaginary (far-pointer) destination reached the same null store from the other direction. Carried as fork patch `0003` — the retired `0003-late-opt-txy-dead-flag` (→ PR #562) slot and lifecycle; **verified `patch -p1 --dry-run` clean against pristine `8be054612`**. | [PR body](upstream-late-opt-nongpr-ldimm-pr.md) (incl. live-demo sighting link + full post runbook: push, `gh pr create`, status flip, wald3n.com `task open-source:refresh`) · patch `patches/llvm-mos/0003-late-opt-nongpr-ldimm-dest.patch` · [plan](plans/2026-07-27-138-lzss-far-decode-mos-late-optimization-crash.md) | **POSTED** — [PR #584](https://github.com/llvm-mos/llvm-mos/pull/584), `wbniv:mos-late-opt-nongpr-ldimm` @ `7eedb14a2597` (from tip `8b616af9434f`). 2026-07-31 follow-up after post-hoc review: `7eedb14a2597` hardens the guard (invalidate aliased GPR tracking before the skip — dead code today, survives future class widening); PR body's live-byte claim corrected to the measured matrix (3 live bytes crash at O1+, 2 at `-Oz`; “8 is the smallest” was false) + explanatory PR comment. Fork carry updated in the same commit (`0999cfa`). |
