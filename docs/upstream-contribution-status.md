...
| 18 | **COP mnemonic + BRK/COP signature byte** — assembler has no `cop` at all; `brk` takes no operand | **fix PR** | Stock-llvm-mos assembler/ISA-table gap found by the #140 BRK/COP software-vector demo (which works around it with `.byte $02,$5a` / `.byte $00,$42`): `$02` is absent from the 65816 table entirely and `brk #imm` is rejected, though the W65C816S data sheet lists **both BRK and COP as two-byte instructions** and WDC syntax *requires* COP's signature byte. **Fix** = add `COP_Implied`/`COP_Immediate` (gated `FeatureW65816`, **not** the shared Or65EL02 predicate — `$02` is `NXT` on the 65EL02) + a `BRK_Immediate` form; ca65's optional-operand rule, so bare `brk` keeps its 1-byte encoding and the change is strictly additive. Disassembly deliberately unchanged (1-byte decode preserved; `$02` now reads `cop` not `<unknown>`). Tests: new `brk-cop-signature.s` + additions to `all-65816-opcodes.s` and `asm-errors.s`; MC suite 39/40 with the lone failure proven pre-existing on pristine upstream. **DRAFTED 2026-08-04 — NOT POSTED, BRANCH NOT PUSHED** (user-triggered). | [PR body + post cmd](upstream-cop-brk-signature-pr.md) · [plan](plans/2026-08-04-mos-brk-cop-signature-byte-assembler.md) | not pushed (`mos-65816-cop-brk-signature` @ `61c07100970c`, local in `~/llvm-mos`, cut from `1f334fef02b5`) |
| 19 | **`llvm-mc` clobbers the target's Motorola-integer default** — `$`-hex silently off under bare `llvm-mc` | **fix PR (tiny)** | `AsmLexer` initialises `LexMotorolaIntegers` from `MCAsmInfo::shouldUseMotorolaIntegers()`, and MOS sets `UseMotorolaIntegers = true` (`MOSMCAsmInfo.cpp:45`, llvm-mos #352) — but `llvm-mc.cpp:374` then unconditionally overrides it from a `cl::opt` defaulting to **false**, so `$5a` stops lexing as hex under bare `llvm-mc`. Predates the MOS opt-in (override added 2021-01-26 by `4db18d62afa8`, *[M68k] Motorola literal syntax*). Evidence: **35 of 39** MOS MC tests pass `-motorola-integers` to compensate; the real driver is unaffected (`mos-clang` assembles `wdm #$5a` → `42 5a` unflagged). **Fix** = honour the flag only when given (`getNumOccurrences()`). Touches generic `llvm-mc`, so it needs its own PR + a check that no other target relies on the override. Offered as a footnote in row 18's PR body. | [plan §Side finding](plans/2026-08-04-mos-brk-cop-signature-byte-assembler.md) | not drafted (found 2026-08-04) |
