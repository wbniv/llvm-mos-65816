...
|--------|----------|------|--------|
| `wt/m7-gallery-web-reconcile` | `/home/will/llvm-mos-65816-m7webrec` | **Reconcile the Mode 7 gallery website layer** — plans 121 (gates 11/17/20/22/23) and 123 (steps 2/4/10) re-baselined once ([plan](plans/2026-09-14-m7-gallery-web-reconcile.md)) | **✅ DONE + both plans re-verified (2026-09-14), on the branch — NOT merged.** Hardlink/non-compiler worktree. Docs-only in this repo plus `dev/m7web/` (host-Chrome DevTools harness: `smoke22.mjs`, `filter123.mjs`, `webgates.sh`, `romgates.sh` — no browser tooling in either site repo). 121: 22/23 (only #22 on indri, the tracked `[wip T2]` player defect); 123: 10/10. Gate 20 policy set (demo-source drift → republish; toolchain-only → accepted divergence) and the `mandel-oop` republish (`140c7b74…`, `corpus_result @ 0x897`) **staged, unpushed** on `snes/mandel-oop-republish` in worktrees `~/biohack.net-m7rec` (`e7f7d09`) and `~/indri.studio-m7rec` (`bc7d7cc`). Sibling worktrees are disposable once merged/published. |
