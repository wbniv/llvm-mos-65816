...
|--------|----------|------|--------|
| `wt/321-a16-pressure` | `/home/will/SRC/llvm-mos-65816-a16-pressure` | #321 fix the two `+mos-a16` register-pressure crashes ([plan](plans/2026-06-24-321-a16-pressure-fix-implementation.md) · [handoff](plans/2026-06-23-321-a16-pressure-scavenger-fix-handoff.md)) | **✅ REGPRESS FIXED + committed (`c53c417`) — clean PARTIAL; scavnz/pr15296 stay deferred.** Compiler-changing worktree (own `vendor/` + warm release+asserts `build/`). Asserts `-debug-only=regalloc` pinpoint: the `regalloc-out-of-registers` deadlock is the strength-reduced i8 array byte index (`i += 2`) selecting to A-pinned `ADCImm` (class `Ac`={A}) and blocking the 16-bit indexed-load `Ac16` transit — the counter can't recolor off the singleton {A} and the INF transit can't spill (NOT `Ac16` over-coalescing; coalescing was already ruled out). **Fork patch `0009`** (`dev/regen-patch-0009.sh`, round-trips, 0 foreign hunks): `MOSInstructionSelector::selectAddSub` lowers a small-const i8 add/sub (`\|Amt\|≤2`) to a relocatable `G_INC`/`G_DEC` chain (Anyi8) under `hasAccum16()` so the index lives in X (`inx;inx;cpx`), freeing A16. **DEFAULT byte-identical**; net **−123 B / 122 c-torture progs (0 worse)**; corpus 7/7, a16 suite 57/0, fuzz 50 0-mismatch, `-verify` clean; `a16regpress.c` + original `globals.c` compile+run clean release+asserts; new positive gate `dev/run.sh a16regpress` (0x01A7 MAME+bsnes-jg). De-XFAIL'd `regalloc-out-of-registers`. **scavnz (`scavenger-p-not-gpr`) + `a16-zp-pressure-overflow` (pr15296) NOT addressed** — no i8 byte index, pure native-s16 pressure → the deferred Phase-3 s16/ZP-residency rework (13-agent design workflow + prior spike concur). **Worktree RETAINED — land `0009` to `main`'s shared `vendor/`+patch stack is the pending follow-up (coordinate).** Refuted the design-workflow synthesis's "the +2 is a 16-bit address IV" premise (legalized MIR: `G_ADD s8 ,2`). |
