...
|--------|----------|------|--------|
| `wt/jgxcheck-determinism` | `/home/will/llvm-mos-65816-jgxcheck-determinism` | **`dev/jgxcheck` timeline captures are nondeterministic by default** — pin the harness's own default entropy to None ([plan](plans/2026-09-14-jgxcheck-determinism.md)) | **✅ DONE + verified (2026-09-14), on the branch — NOT merged.** Hardlink/non-compiler worktree (shares `main`'s prebuilt toolchain + `vendor/bsnes-jg` via `cp -al`; no rebuild). `jgxcheck.cpp:394` now defaults `SuperFamicom::configuration.entropy` to `0` (None) when `JGX_ENTROPY` is unset, instead of inheriting bsnes-jg's own clock()-seeded Low — fixes the ~150 call sites that never touched the var without editing any of them; the 5 sites that already pass `JGX_ENTROPY` explicitly are no-ops. `dev/bootblank.sh --firstframe` (the one gate that deliberately wants live entropy) gets an explicit `JGX_ENTROPY=1`. **Verified:** two back-to-back PNG captures of `hello.sfc` byte-identical under the new default; `dev/run.sh {a16abs,absdiff,xy16inplace}` all PASS with unchanged oracle hashes (`0x5A3D`/`0x3482`/`0x90AA`+`0xDEBD`); raw WRAM at an address `hello.c` never writes reads a different clock()-seeded value on every run under explicit `JGX_ENTROPY=1` (`0xF911`/`0x1C1C`/`0x6A9C`/`0x0606`) and a pinned `0x0000` on every run under the new default — direct proof the override still injects live entropy while the default is now deterministic. **Worktree not yet torn down** — pending merge to `main`. |
