...
|--------|----------|------|--------|
| `wt/320-far-followups` | `/home/will/SRC/llvm-mos-65816-far-followups` | #320 far-calls follow-ups: (a) far function pointers + (b) mixed-banking (far→near) ([plan](plans/2026-06-21-320-far-calls-followups.md)) | **(b) DONE 2026-06-21 (`dd33017`), (a) BLOCKED on `0004`.** Compiler-changing worktree (own `vendor/` + warm `build/`). **(b) far→near** = generic bank-0 thunk **`__call_near_from_far`** (`pea .Lback-1; jmp (__rc18); rtl`) reached by `JSL`; `lowerCall` materializes `&g`→RS9, `ChangeToES`, emits JSL (HasW65816-gated, a16-free, in `0001`). Verified `far_near_call == 0xE0` MAME+bsnes-jg, corpus 7/7, thunk gc'd from near ROMs (byte-identical), prior far ROMs PASS, `0001` round-trips. **(a)** re-confirmed BLOCKED: forming/returning/storing a `p2` value crashes (`G_TRUNC`/`G_UNMERGE`/`G_STORE` p2) — the p2-VALUE class the far-CC study shipped as Imag32 in **`0004-320-far-cc.patch`** (on `wt/320-far-cc`, not on `main`). (a) = front-end (F1/F2/F3) + the `0004` base + `__call_indir_far`/indirect lowering + residual legalizer fixes; **gated on `0004` reaching `main`**. ⚠ worktree's `vendor/` carries a foreign `MOSInsertREPSEP.cpp` WIP delta (copied at `cp -a` setup; excluded from `0001`, left per policy). |
