...
|--------|----------|------|--------|
| `wt/320-far-cc` | `/home/will/SRC/llvm-mos-65816-far-cc` | far-pointer calling convention: build all ABI variants & measure ([plan](plans/2026-06-20-320-far-pointer-cc-build-all-variants.md)) | 🔬 **IN PROGRESS** (2026-06-21): P0 + A0 (variant a Imag32) + A1 (variant b Imag16+bank) + **A2 (variant c A:X+Y)** landed (`10a5fc0`, `741a8c2`, `02953e7`) — each round-trips arg+return to `0xF3` on MAME + bsnes-jg; `0004-320-far-cc.patch` captures all three (429 lines, round-trip-verified). Remaining: A3 (variant d stack — only the inert feature/predicate stub exists; `,S` modes are 65CE02-gated), M (byte+cycle measurement — **cycle harness is net-new, not built**), D (promote winner → `0001`, default-on). All variants gated off-by-default `+mos-farcc-*` in spike patch **`0004`**. **Branch LIVE — do NOT tear down.** |
