...
| `wt/320-packed24-incB` | `/home/will/SRC/llvm-mos-65816-packed24-incB` | #320 packed-24 (AS3) **Increment B** — codegen to store/load/deref a 3-byte packed far pointer ([handoff](plans/2026-06-21-320-packed24-incrementB-handoff.md) · [plan §Build packed-24](plans/2026-06-21-320-five-address-space-model.md)) | **✅ DONE + verified 2026-06-21** (compiler-changing worktree off post-F2 `main`; own `vendor/` + warm-copied `build/`). F2 precondition gate PASS first (`sizeof(far*)==4`; far store/load/array/struct legalize clean). **Increment B was NOT the predicted s24-narrowing job** — two findings: (1) `CodeGenPrepare::optimizeLoadExt` crashed on the invalid `MVT::i24` for a 24-bit pointer load → fixed by `getPointerTy(AS_FarPacked)→i32` (the packed ptr's register form IS the 32-bit far form; memory footprint stays 3 B via datalayout); (2) the artifact combiner doesn't look through `inttoptr/ptrtoint`, so route `p3↔3×s8` via `G_MERGE/G_UNMERGE{PFP,S8}` (no `s24`), which **folds** against the adjacent unmerge/merge in every shape clang emits (cast→store, load→cast, packed→packed copy) → no 24-bit value reaches selection. Shipped as stacked **`0006-320-packed24.patch`** (regen: `dev/regen-patch-0006.sh`; NOT folded into 0001 — packed-24 edits files 0004/0005 also touch). **Verified:** `dev/run.sh packed24` (new) `corpus_result=0xF3` on MAME **and** bsnes-jg — far ptr targets **bank $01**, so 0xF3 proves the bank byte survives 3-byte packing; `-verify-machineinstrs` clean; **corpus 7/7**; far suite (bank1/cast/arith/store/call) PASS; `fuzz 50` 0-mismatch; storage **48 B vs 64 B (−16 B/−25%)** for a 16-entry table, ×3 index cost. New e2e `examples/65816/packed24/packed24_e2e.c` + `dev/packed24.sh`, wired into `dev/xcheck.sh`. **✅ Task A measure + static-init table fix DONE (2026-06-22, `a76bf18`, pushed):** Task A (`dev/run.sh measure-packed24`) → packed wins **≈N B at every table size, break-even N≥1** (indexed-walk access code is *equal* — far loads only 3 of its 4 entry bytes; the ×3-vs-×4 stride is a constant; the byte-2-long cost only hits *direct single-slot* access, not indexed tables). Task A surfaced that a **statically-initialized** packed table didn't link (each 3-byte entry emitted one `R_MOS_ADDR8` — no 3-byte data fixup, `getDataKindForSize(3)` unreachable → `FK_Data_1`); **fixed** via a generic `AsmPrinter::emitNonStandardSizedConstant` hook + a MOS override emitting the `ADDR24 SEGMENT_LO/HI/BANK` triple (specifier on the `MCSymbolRefExpr`, mirroring the jump-table bytes). Updated **`0006`** (now 10 files incl. the two generic AsmPrinter files, added to `regen-patch-0006.sh`'s `PACKED_FILES`); new `dev/run.sh packed24_table` `0xA5` MAME **and** bsnes-jg (each static entry's bank byte survives), corpus 7/7, fuzz 0-mismatch, round-trip clean. [fix plan](plans/2026-06-22-320-packed24-static-init-reloc-fix.md). **Worktree RETAINED until upstream merge** (user policy). (`far_near_call` fails to *link* here — `__call_near_from_far` lives in the SNES platform lib, which the warm SDK predates; pre-existing stale-artifact, unrelated.) |
| `wt/320-near-abs-bank-relax` | `/home/will/SRC/llvm-mos-65816-near-abs` | #320 near-abs bank-relaxation: suppress `abs → abs-long` for **near (bank-0) globals** so a plain A-register access compiles to `abs` (3 B, `R_MOS_ADDR16`) not `abs-long` (4 B, `R_MOS_ADDR24`); also shortens native-s16 ALU abs ops reading near globals | **Built + verified by a concurrent session (2026-06-22); patch `0007-65816-near-abs-bank-relax.patch` on the worktree, NOT yet on `main`** — a pending landing (edits `MOSAsmBackend.cpp` only; does **not** stack into `0006`). ⚠ The worktree's toolchain binary briefly carried this fix baked-in while the source was reverted — rebuild before measuring in that tree. |
