...
|--------|----------|------|--------|
| `wt/fork-patch-followups-2026-09-13` | `/home/will/llvm-mos-65816-fork-patch-followups` | **Fork-patch follow-ups for the 2026‑09‑13 PR revisions** ([plan](plans/2026-09-14-fork-patch-followups.md)) | Compiler-changing worktree (own `vendor/` + `cp -a` warm `build/`). Lands the published #578/#584/#586/#589/#590 forms in the fork: `0010` = the `MOSCopyOpt` `fullyRecomputeLiveIns` fix + 2 lit tests, now applied by `toolchain.sh` after `0002` (the `shouldCoalesce` Ac guard is gone from `0002`); `0022` = terminator form; `0003`/`0024`/`0021` re-synced; RA companion draft deleted; six PR-body mirrors re-synced. Verification + the `0021` pin-bump procedure in the plan. On the branch — NOT merged. |
| `wt/svx2-anchors` | `/home/will/llvm-mos-65816-svx2-anchors` | **svx2 plan anchors — restore-or-retire decision** ([plan](plans/2026-09-14-svx2-anchors-decision.md)) | **✅ DECIDED + verified (2026‑09‑14), on the branch — NOT merged.** Hardlink/non-compiler worktree. (a) **restored** the 4‑frame LoROM first cartridge: `reel_stream_bank()` guarded by `#ifdef VIDEO_REEL_PACKED_FAR` (culprit `d6030cf`, not `bd344a1`), plus an in-tree recipe for its input — `tools/snes-video-reel-extract.py` inverts the packed-far generator (frames 600–603 of the checked-in 900‑frame header + stream, CRC-checked) and `dev/snes-video-lorom-fixture.sh` is the one-command gate (32 KiB LoROM, 399 presentations exact, fidelity 93.05 %/MAE 1.31, checksum PASS). (b) **retired gate 7** in the svx2 plan — the plan owns the LoROM fixture; "published == gated" belongs to `dev/svx2-emulator-validation.sh` under the artemis‑2x‑apollo plan. (c) **retired** the 1,800‑frame Artemis cadence‑1 cartridge anchor in the cut-aware-labels plan (no asset recipe, unpublished, superseded). **Also fixed** `dev/snes-video-reel.sh`'s cadence gate: it now measures t0 (first present) in-run and derives the expectation instead of hardcoding per-cadence literals — the LoROM literals from `4e93daa` encoded t0 = 408, the fixture measures 404 with zero slips at all three cadences (`b1afb9c` class); the FPS-gauge early probe is `t0 + 68`, not a fixed 400. 900‑frame HiROM default path re-verified (t0 = 179 → `0x777`). Do **not** `git worktree remove` — use `dev/worktree-teardown.sh`. |
