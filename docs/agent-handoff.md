...
|--------|----------|------|--------|
| `wt/119-gallery-near-decode-abi` | `/home/will/llvm-mos-65816-gallery-repack` | #119 **`lzss-gallery` per-work repack self-check failures** — root-cause + fix ([plan](plans/2026-07-30-gallery-near-decode-abi-clobber.md)) | **✅ DONE + verified (2026-07-30), NOT yet merged to `main`.** Hardlink/non-compiler worktree (shares `main`'s prebuilt toolchain via `cp -al`). **Verdict: demo bug, NOT a miscompile.** `decode_bank7e`'s `lda #$7e / pha / plb` clobbered `decode_near`'s `slen`, which the MOS convention passes in `A:X` → every stream with `(lz_len & 0xFF) > $7E` truncated to `(lz_len & 0xFF00) \| $7E` (**29 of 62 works**); the un-decoded tail of `FB_A` then fed the compressor. Fixed with the A-safe `PEA $7E7E` / `PLB` / `PLB` idiom. **Verified (bsnes-jg):** works 0–3 `done=[0,1,2,3] failed=[]` with `last_z == lz_len`; `FB_A`/`FB_B`/`PACK` byte-identical to the assets; new all-62-work `GALLERY_BENCH_ONLY` decode gate `0x5CF0` (and `0xA50F` on the pre-fix thunk); new `decode_bank7e` ABI opcode audit rejects the pre-fix ROM. Two regression guards added to `dev/lzss-gallery.sh`. **Merge is the orchestrator's call** — `main`'s `dev/lzss-gallery.sh` has another worker's uncommitted `FRAMES` change (untouched here). |
