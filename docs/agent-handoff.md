...
|--------|----------|------|--------|
| `wt/display-first-frame-optin` | `/home/will/llvm-mos-65816-dispoptin` (+ a throwaway measurement tree `/home/will/llvm-mos-65816-dispoptin-base`, detached at `22f18cb`, holding the "before" half of the sweep) | **Per-drawable "first frame is complete" opt-in for `snesgfx` Display** ([plan](plans/2026-09-14-display-first-frame-optin.md)) | **✅ IMPLEMENTED + verified (2026-09-14), on the branch — NOT merged.** Hardlink/non-compiler worktree (shares `main`'s prebuilt toolchain via `cp -al`; no `vendor/` rebuild). The safe shape of the blanket fix rejected in [2026‑08‑05](plans/2026-08-05-display-first-frame-forceblank.md): `Drawable.first_frame_complete`, cleared by `drawable_reserve()` and set by a `reserve()` that painted its whole first frame *including CGRAM*; `Display.ff_all` ANDs it in `display_add`; `display_frame` guards the first `scene_emit` on it (a guard, never a reorder — SNESDQ's four-token order is intact). Adopters: `MandelLayer`, `CaDisplay`, `LifeGrid`; `mandel-oop` swaps its demo-local Option J latch for the shared mechanism at **5 → 5** post-title frames (which is the proof the gate fires — without it, deleting the latch would have restored 11). **Verified:** SNESDQ PASS; `dev/bootblank.sh --firstframe` 117 demos **0 new** nondeterministic (same 4 pre-existing on both trees); `dev/bootblank.sh` **0 of 117** boot-window frames moved; `dev/m7blank.sh --gate` all 12 ok; `dev/run.sh mandel-oop` `0x204F` MAME+bsnes-jg, 1 indirect dispatch. **Known deviation:** non-adopting ROMs are NOT byte-identical (a runtime gate compiles into every demo, ±tens of bytes); behavioural identity carries the "safe by default" claim instead. Follow-up with measurements already attached: `TitleLayer` needs a CGRAM[0] write in its `reserve()` to qualify (worth −1 frame on `life`, 0 on `1d-ca`). |
