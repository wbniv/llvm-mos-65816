...
|--------|----------|------|--------|
| `wt/320-far-followups` | `/home/will/SRC/llvm-mos-65816-far-followups` | #320 far-calls follow-ups — **combined plan+handoff: [plans/2026-06-21-320-far-calls-followups.md](plans/2026-06-21-320-far-calls-followups.md)** (read it to resume) | **(b) far→near DONE + SHIPPED to `main` (`5717f6b`)** — generic bank-0 thunk `__call_near_from_far`; verified `0xE0` MAME+bsnes-jg, corpus 7/7, gc'd from near ROMs. **(a) far fn pointers IN PROGRESS** (worktree retained): call MECHANISM built+verified (i32-target) — `lowerCall(__call_indir_far)→JSL` + stub `jml (__mos_far_target)`; IR-rep #1 (volatile slot store + thunk), front-end = F2. p2-value path = a deep multi-layer `0004` sub-project: ✅ L1 `copyCost` Imag32 case, ✅ L2 `getRegAllocationHints` size-guard (both regression-clean), ⏳ L3 `SelectImm` illegal Imag8 cond, ⏳ Gap B `G_STORE p2`, ⏳ Gap A `&far_sym`→24-bit, ⏳ clang F2, ⏳ e2e. Root-cause via `dev/run.sh asserts-build`. The `lowerCall` + 2 `MOSRegisterInfo` vendor edits are recipes in the plan; `0004` stacked here; ⚠ foreign `MOSInsertREPSEP.cpp` WIP delta in `vendor/` (leave). **Full state + resume steps: the combined doc.** |
