...
|--------|----------|------|--------|
| `wt/320-far-followups` | `/home/will/SRC/llvm-mos-65816-far-followups` | #320 far-calls follow-ups: (a) far function pointers + (b) mixed-banking (far→near) ([plan](plans/2026-06-21-320-far-calls-followups.md)) | **(b) DONE + SHIPPED to `main` (`5717f6b`), (a) PAUSED (gated on `0004`).** Compiler-changing worktree (own `vendor/` + warm `build/`); **RETAINED** for (a). **(b) far→near** = generic bank-0 thunk **`__call_near_from_far`** (`pea .Lback-1; jmp (__rc18); rtl`) reached by `JSL`; `lowerCall` materializes `&g`→RS9, `ChangeToES`, emits JSL (HasW65816-gated, a16-free, in `0001`). Verified `far_near_call == 0xE0` MAME+bsnes-jg, corpus 7/7, thunk gc'd from near ROMs (byte-identical), prior far ROMs PASS, `0001` round-trips. **(a)** BLOCKED on a design decision, not just impl. (i) p2-VALUE crashes (`G_TRUNC`/`G_UNMERGE`/`G_STORE` p2) = the class far-CC shipped as Imag32 in **`0004-320-far-cc.patch`** (stacked into this worktree to try (a); not on `main`). (ii) **⚠ Layer-3 (measured 2026-06-21): a far fn ptr CANNOT be a `ptr addrspace(2)` callee** — LLVM forbids a non-program-addrspace callee (verifier "expected 'ptr'"; no `P<n>` in the MOS datalayout); `addrspacecast` p2→p0 drops the bank. So a "detect p2 callee in `lowerCall`" trigger is **untriggerable** (prototyped + backed out). **IR-rep #1 CHOSEN (user) + building end-to-end**, realized as a volatile store to a runtime slot `__mos_far_target` + `call @__call_indir_far` (lighter than a formal intrinsic, which needs `Intrinsics.td` regen). **The far-indirect call MECHANISM is BUILT + verified (2026-06-21):** `lowerCall` makes a call to `__call_indir_far` a `JSL`; stub `platforms/snes/call-indir-far.s` = `jml (__mos_far_target)` + the 4-byte bank-0 slot; hand-IR with an **i32 target** → slot store + `jsl __call_indir_far`, `-verify-machineinstrs` clean. **Remaining (substantial) — p2-value legalization is a DEEP MULTI-LAYER 0004 sub-project** (asserts-build root-caused): ✅ Layer 1 `copyCost` missing `Imag32` case; ✅ Layer 2 `getRegAllocationHints` size-mismatch guard (both **regression-clean** — corpus 7/7, far_near_call PASS — only fire for imag32/p2); ⏳ Layer 3 `SelectImm` with an Imag8 condition (`$rs1 = SelectImm $rc5` → verify "Illegal physical register"); ⏳ Gap B `G_STORE p2`; ⏳ **Gap A** (`&far_sym`→24-bit). Then clang **F2** `far` attr + CodeGen (store+call), then the e2e gate. The `lowerCall` (3-line) + the 2 `MOSRegisterInfo` fixes live in the worktree's `vendor/` (recipes in the plan). Resume gated on the p2-legalization work + `0004` reaching `main`. ⚠ worktree `vendor/` also carries a foreign `MOSInsertREPSEP.cpp` WIP delta (from `cp -a` setup; excluded from `0001`, left per policy). |
| `wt/320-far-cc` | `/home/will/SRC/llvm-mos-65816-far-cc` | #320 Inc 4 Ph2: far-pointer calling convention — build all 4 ABI variants (a Imag32 / b Imag16+bank / c A:X+Y / d hw-stack) & measure ([plan](plans/2026-06-20-320-far-pointer-cc-build-all-variants.md)) | **IN PROGRESS** 2026-06-21: compiler-changing worktree (own `vendor/` + warm-copied `build/`). **P0 + A0 (variant a Imag32) DONE & two-emulator verified** (`10a5fc0`): a 32-bit far ptr returned-from + passed-into noinline calls round-trips `0xF3` on MAME + bsnes-jg; default byte-identical; csmith 30 seeds 0-mismatch. Needed `Imag32` ∈ `AnyRegBank` (COPY-through class-constraint). Far-CC delta = stacked **`0004-320-far-cc.patch`** (not `0001` — shares the `AnyRegBank`/`Ac16` line with `0002`). **Pending:** A1–A3 (variants b/c/d) · M (bytes+cycles harness) · D (ship winner). Only the WINNER lands; losers stay a measured spike. |
