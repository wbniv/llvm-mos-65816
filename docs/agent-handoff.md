...
|--------|----------|------|--------|
| `wt/321-dpy-indexed-phase2-increment2` | `/home/will/llvm-mos-65816-dpy-phase2-inc2` | Range-proven runtime far `[dp],Y` indexing ([plan](plans/2026-09-25-dpy-indexed-phase2-increment2.md)) | Rebased onto `8c19c703`; committed locally as `97798007`. Focused lit, full native corpus, 45-seed fuzz (5 harness skips), farblit, near-store controls, BankWalk and round trips pass. The three full-lit failures reproduce on the exact baseline. Include in the full #321 ABI-documented feature series PR. The [near-Y gallery failure](defects/mos-xy16-near-indirect-y-clobber.json) is separate and remains open. |
