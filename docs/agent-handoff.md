...
|--------|----------|------|--------|
| `wt/320-far-followups` | `/home/will/SRC/llvm-mos-65816-far-followups` | #320 far-calls follow-ups: (a) far function pointers + (b) mixed-banking (far→near) ([plan](plans/2026-06-21-320-far-calls-followups.md)) | **IN PROGRESS** 2026-06-21: compiler-changing worktree (own `vendor/` + warm-copied `build/`; corpus 7/7 on the warm toolchain). Sequence: (b) far→near first (backend-only, no front-end), then (a) backend (`__call_indir_far`), then (a) front-end (decision F1/F2/F3). **(b) design refined to a generic bank-0 thunk `__call_near_from_far`** (`pea .back-1; jmp (__rc18); rtl`) reached by `JSL` — shares the copy-addr→slot+JSL-to-stub plumbing with (a), avoids AsmPrinter veneer synthesis; per-callee veneer is a future byte-opt. Lands in `0001` (HasW65816-gated, a16-free). |
