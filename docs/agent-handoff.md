...
|--------|----------|------|--------|
| `wt/321-mandel-zoom` | `/home/will/SRC/llvm-mos-65816-mandel-zoom` | #321 **Mandelbrot ZOOM PYRAMID** — true increasing detail on zoom-in ([plan](plans/2026-06-25-321-mandelbrot-zoom-pyramid.md)) | **✅ Phase 1 DONE+green (`a54041b`, on the branch — NOT merged).** Hardlink/non-compiler worktree (`build/install` real-copied so `sync-platform`'s `cp -f` can't mutate `main`). Host bakes a stack of 2×-finer Mandelbrot levels (`tools/mandel-bake-pyramid.c`, `double`, centre = real-axis mini-Mandelbrot); the ROM DMAs the next finer level as Mode-7 zoom crosses each 2× threshold — increasing detail, zero on-console fractal math. Single-bank proof: 64×64 × 6 levels (32×) in one 32 KiB bank, builds default+`+mos-a16`. New `examples/snes/{zoom.h, mandel-zoom.c}`, `dev/mandel-zoom.sh`, `mode7.h` param `m7_tilemap_identity`, `jgxcheck.cpp` `JGX_ZOOM`. **Differential PASS** (`dev/run.sh mandel-zoom`): per-level hash all 6 host==default==`+mos-a16` (0x9191+HASH) MAME+bsnes-jg; scripted-zoom view-math host==target both builds; `-verify` clean; 32 KiB fit. Regressions green (`mandel-interactive` 0xF99C, `mandel-mode7` 0x75E8). **FINDING: a real DEFAULT-8bit matrix-fold-LOOP miscompile** (`for(i<4) m[i]` folds 0x456E vs correct 0xB115; unrolled is correct; context-sensitive, INDEPENDENT of #321 — TODO follow-up for a cvise→backend fix). **Phase 2 (multi-bank 128×128×8) feasibility PROVEN** (bank-$01 DMA spike `docs/plans/spikes/2026-06-25-321-mandel-zoom-phase2-bank-dma-probe.c`) + documented as a de-risked follow-up. **Worktree RETAINED** (user policy); merge user-triggered. |
