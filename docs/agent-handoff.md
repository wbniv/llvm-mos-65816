...
|--------|----------|------|--------|
| `wt/320-far-cc` | `/home/will/SRC/llvm-mos-65816-far-cc` | far-pointer calling convention: build all ABI variants & measure ([plan](plans/2026-06-20-320-far-pointer-cc-build-all-variants.md)) | 🔬 **IN PROGRESS** (2026-06-21): P0 + A0 (variant a Imag32) + A1 (variant b Imag16+bank) landed (`10a5fc0`, `741a8c2`), both round-trip arg+return on MAME + bsnes-jg. **A2 (variant c A:X+Y) is live in `vendor/` but uncommitted** — its `CC_MOS_FarPtrAXY` rule/assigner are **not in `0004` yet** and `dev/farcc_axy.sh` is untracked (regen `0004` + commit before trusting the patch). A3 (variant d stack), M (byte+cycle measurement — cycle harness is net-new, not built), D (promote winner → `0001`, default-on) pending. All variants gated off-by-default `+mos-farcc-*` in spike patch **`0004-320-far-cc.patch`**. **Branch LIVE — do NOT tear down.** |
