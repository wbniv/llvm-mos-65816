...
| `wt/320-five-space` | `/home/will/SRC/llvm-mos-65816-five-space` | packed-24 (AS3) — the five-space size-opt space ([plan §Build packed-24](plans/2026-06-21-320-five-address-space-model.md)) | **Increment A DONE; Increment B blocked on 24-bit width** 2026-06-21. Compiler-changing worktree (own `vendor/` + warm `build/`, off `main`). **A (the 3-byte TYPE):** `AS_FarPacked=3` + datalayout `p3:24:8` + clang `getPointerWidthV` case 3 → `sizeof(packed*)==3`, `table[16]==48 B` (vs 64), **corpus 7/7** (AS3 inert otherwise). **B (codegen to USE packed ptrs): BLOCKED** — `addrspacecast p2↔p3` + `load/store p3` route through `s24`, which the backend doesn't legalize (only `s8/s16/s32`; empirical *"extend/truncate can not operate on pointers"*), and 3-byte granularity breaks 2-source merges → a genuine novel-s24-width GISel effort, **deferred** (speculative opt; no real far-ptr-table pressure). Recipe+blocker in the plan §Build packed-24. RETAIN per worktree policy. |
