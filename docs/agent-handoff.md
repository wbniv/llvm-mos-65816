...
|--------|----------|------|--------|
| `wt/320-far-cc` | `/home/will/SRC/llvm-mos-65816-far-cc` | far-pointer calling convention: build all ABI variants & measure ([plan](plans/2026-06-20-320-far-pointer-cc-build-all-variants.md), [M+D](plans/2026-06-21-320-far-cc-M-measure-D-land-winner.md)) | ✅ **RESOLVED (2026-06-21)** — all four variants built + measured; **D shipped (a) Imag32 as the DEFAULT far-ptr CC** (`b2bcdd1`): a far pointer crosses a call with **no flag** now (it errored before). Measured (.text / round-trips-per-120f): **(a) 70 / 50441 — smallest AND fastest**; (b) 86 / 41385; (c) 102 / 43572; (d) 174 / 30626. Commits `10a5fc0` P0+A0, `741a8c2` A1, `02953e7` A2, `ebaa515` A3a, `ceaded6` M, `b2bcdd1` D. b/c/d remain the inert opt-in `0004` spike; **variant (d) hardware-`,S` recorded-and-dropped** (`SPAdj==0` + missing 65816 `,S` opcodes; frame-ABI 0/13). Verified: no-flag == winner, corpus 7/7, all 12 far ROMs `0xF3` both emulators. **Branch RETAINED (holds the gated spike) — do NOT tear down.** Upstream #320 CC evidence note queued (user-triggered). |
