...
Propagate live-ins to a fixed point with `fullyRecomputeLiveIns` before dead-copy cleanup, and once more afterwards so live-ins and kill flags match the erased copies. Keep the entry block's ABI live-ins intact. Remove the original shouldCoalesce workaround; ordinary rotate coalescing remains enabled. The live-in expectations in `copy-opt.mir` change in order only, since recomputed live-ins are sorted.
