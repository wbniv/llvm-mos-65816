...
| --- | --- | --- |
| #578 | `edc9bbd23b71` | `019af10c7378` (was `c4fd3dfa74e7` before the review follow-up) |
...
- **#578** `MOSCopyOpt.cpp`: the per-block `clearLiveIns`/`computeAndAddLiveIns` tail is replaced by one
  `fullyRecomputeLiveIns` plus a flag recompute after the dead-copy erase loop. `copy-opt-loop.mir`
  gains an explicit `BRA %bb.3`. `copy-opt.mir`'s two live-in CHECK lines change in order only, because
  `recomputeLiveIns` sorts live-ins. Re-validated on a rebuilt llc: both `copy-opt-loop.mir` prefixes pass;
  CodeGen/MOS **79 pass**, 1 unsupported; all **176** `-O2` corpus outputs from `examples/65816/*.c`
  (IR from the fork's clang, two feature variants each) are **byte-identical** to the previous
  `c4fd3dfa74e7` build. The bsnes runtime check was **not** re-run for the collapsed form; the corpus
  identity is the equivalence evidence.
