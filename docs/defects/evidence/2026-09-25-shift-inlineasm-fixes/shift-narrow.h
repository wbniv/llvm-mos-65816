...
static volatile uint8_t shift64seam_count;
static uint64_t shift64seam_left_out;
static uint64_t shift64seam_right_out;
static int64_t shift64seam_arith_out;

__attribute__((noinline)) static uint64_t shift64seam_left(uint64_t x, uint8_t count) {
    return x << count;
}

__attribute__((noinline)) static uint64_t shift64seam_right(uint64_t x, uint8_t count) {
    return x >> count;
}

__attribute__((noinline)) static int64_t shift64seam_arith(int64_t x, uint8_t count) {
    return x >> count;
}
