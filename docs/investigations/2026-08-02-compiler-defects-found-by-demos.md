...
| 12 | **`LDCImm` set-lowering assert** — baseline MOS MC lowering accepted only `0`/`-1`, although its i1 immediate can be plain `1`. | surfaced while validating the scavenger fix (compilation reached MC lowering once the scavenger stopped crashing) | assert / release-mode UB | fixed (`0012`); baseline `mos65c02` MIR regression; status row 10 |
