...
/* Spin until the v-blank NMI flag latches (RDNMI bit 7, read-clears). Needs NMITIMEN bit 7
 * (NMI enable) set so the flag is raised each frame. The cheapest "wait for vblank" before
 * touching VRAM/CGRAM/OAM. */
static inline void snes_wait_vblank(void) {
  while (!(REG_RDNMI & RDNMI_FLAG)) { }
}
