...
KNOWN_ISSUES = [
    ...
    # held across a 2nd loop over u16 arrays — was FIXED 2026-06-24 by fork patch 0009: the
    # strength-reduced i8 byte index (`i += 2`) used to select to A-pinned ADCImm (class Ac={A})
    # and deadlock against the 16-bit indexed-load Ac16 transit; selectAddSub now lowers a
    # small-constant i8 add/sub to a relocatable G_INC/G_DEC chain (Anyi8) under +mos-a16 so the
    # index lives in X. Its KNOWN_ISSUES entry + KNOWN_ISSUE_REPROS row are removed so a recurrence
    # hard-FAILS again; the repro examples/65816/a16regpress.c is now a positive gate
    # (dev/run.sh a16regpress, corpus_result==0x01A7). See
    # docs/plans/2026-06-24-321-a16-pressure-fix-implementation.md.)
    ...
    # same +mos-a16 single-accumulator pressure family as the now-fixed regalloc-out-of-registers,
    # but a DISTINCT trigger: pr15296.c is pointer/union/intptr_t register pressure with no i8 byte
    # index, so fork patch 0009 (the i8 small-const inc/dec relocation) leaves it byte-identical and
    # it STILL overflows the ZP addressing budget at link. Surfaced by the c-torture gate (pr15296.c).
    # Classified here so torture_run XFAILs it (the fuzzer never feeds link errors to
    # classify_known, so its behavior is unchanged). The remaining fix is the deferred Phase-3 Ac16/
...
KNOWN_ISSUE_REPROS = [
    # (a16regpress.c / regalloc-out-of-registers dropped 2026-06-24 — FIXED by fork patch 0009;
    # the repro is now a positive gate, dev/run.sh a16regpress.)
...
def run_one(seed, want_bsnes=True):
...
def cmd_run(args):
    ...
    print("==> a16 differential fuzz: %d program(s), seeds %d..%d" % (total, base, base + total - 1))
    print("    toolchain=%s  bsnes=%s" % (TOOL, "yes" if (JGX.exists() and JG_DB.is_dir()) else "no"))
    for i in range(total):
        seed = base + i
        try:
            status, msg = run_one(seed, want_bsnes=not args.no_bsnes)
        except Exception as e:  # never let one program kill the run
            status, msg = "ERROR", "exception: %r" % e
            try:
                triage(seed, Gen(seed).program() and Gen(seed).emit_c(), "driver exception", {"error": repr(e)})
            except Exception:
                pass
