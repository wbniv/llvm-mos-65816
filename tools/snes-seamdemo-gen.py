...
def emit_decode(b: Build) -> str:
    """The full decode-cell table, for the HOST harness only.

    Act 2's node addresses are deliberately a mix of canonical and MIRROR CPU
    addresses -- that is the whole point of the act -- so a reader needs the
    mirror-aware decode.  The console gets it free from the cartridge's address
    decoder; `tools/seamdemo-sim.c` does not, and must not re-derive the mapping
    (that is `snes_cartmap.py`'s job and nothing else's).  So the model emits it.

    Kept in its own header rather than seamdemo-data.h: the ROM must never link
    a 380-entry table it has no use for."""
    cells = b.cm.decode_cells()
    L: list[str] = []
    A = L.append
    A("/* GENERATED by tools/snes-seamdemo-gen.py from tools/snes_cartmap.py -- do not edit.")
    A(f" * Every 32 KiB CPU view of {b.cm.mapping} 0x{b.cm.size:X}, mirrors included.")
    A(" * HOST-ONLY: the console decodes in hardware and must not link this. */")
    A("#ifndef SEAMDEMO_DECODE_H")
    A("#define SEAMDEMO_DECODE_H")
    A("")
    A(f"#define SEAMDEMO_CELL_COUNT {len(cells)}")
    A("static const unsigned char seamdemo_cell_bank[SEAMDEMO_CELL_COUNT] = {")
    for i in range(0, len(cells), 16):
        A("  " + ", ".join(f"0x{c.bank:02X}" for c in cells[i:i + 16]) + ",")
    A("};")
    A("/* 0 = $0000-$7FFF, 1 = $8000-$FFFF */")
    A("static const unsigned char seamdemo_cell_half[SEAMDEMO_CELL_COUNT] = {")
    for i in range(0, len(cells), 32):
        A("  " + ", ".join(str(c.addr_lo >> 15) for c in cells[i:i + 32]) + ",")
    A("};")
    A("static const unsigned long seamdemo_cell_file[SEAMDEMO_CELL_COUNT] = {")
    for i in range(0, len(cells), 8):
        A("  " + ", ".join(f"0x{c.file_start:06X}UL" for c in cells[i:i + 8]) + ",")
    A("};")
    A("")
    A("#endif")
    A("")
...
def emit_header(b: Build) -> str:
    ...
    A("/* Progression values for a partially-built cartridge: the SAME fold, over")
    A(" * the acts that exist yet.  P1 shipped act 1 alone, P2 ships ACT12, P3")
    A(" * replaces it with SEAMDEMO_CORPUS_RESULT. */")
    A(f"#define SEAMDEMO_CORPUS_ACT1 0x{ora.corpus_fold(r['act1']['crc']):04X}u")
    A(f"#define SEAMDEMO_CORPUS_ACT12 "
      f"0x{ora.corpus_fold(r['act1']['crc'], r['act2']['crc']):04X}u")
...
def main(argv: list[str]) -> int:
    ...
    for name in ("emit-header", "emit-decode", "report", "fill", "check-extents"):
    ...
    if args.cmd == "emit-decode":
        with open(args.out, "w") as f:
            f.write(emit_decode(b))
        print(f"decode {args.out}: {len(b.cm.decode_cells())} cells (host harness only)")
        return 0
