...
- [x] **#321 `corpus-a16` in CI — VERIFIED green (run `27823207476`, 2026-06-19).** Secret-gated
  `corpus-a16` step in `smoke.yml`'s `xcheck` job runs the SNES corpus under `+mos-a16`/`+mos-xy16`
  (MAME + bsnes-jg, `globals` XFAIL) on a clean GH runner — its first real CI run (the prior green runs
  predated this step). [plan](docs/plans/2026-06-19-321-corpus-a16-ci.md).
- [x] **#321 bsnes-jg `xcheck` in CI — VERIFIED green (run `27823207476`, ~1h46m cold; cached after).**
  The `xcheck` job in `smoke.yml` builds the from-source patched toolchain (`actions/cache@v5`) + snes-far
  SDK, then `dev/run.sh xcheck` on bsnes-jg — the heavy job's first real CI validation. Watch any run live
  with `dev/ci-watch.sh` / `task ci-watch`.
