...
- [ ] **#320 Inc 3c — runtime far-pointer arithmetic** (`G_PTR_ADD` on AS2). Blocked on the symmetric
  **`s32→4×s8 G_UNMERGE_VALUES`** legalization (mirror `legalizeMergeS32FromBytes`, `hasAccum16`-gated,
  belongs in `0002`) — `far_arith.c` (the gate) is the first to hit it. Once that lands, unskip 3c:
  add the `far_arith` micro-test + xcheck row. (3a `[dp]` deref + 3b AS0→AS2 cast shipped in `0001`.)
  [plan](docs/plans/2026-06-20-320-far-pointer-runtime.md)
- [ ] **#320 far-pointer calling convention** — pass/return a 32-bit `p2` far pointer across function
  boundaries (e.g. `make_far_ptr`). An ABI decision (how `p2` splits across registers); upstream-gated;
  group with far calls (Inc 4). Until then runtime far pointers are usable **within a function**.
