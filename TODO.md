...
- [ ] **#321 native s16 equality-as-value — v2/v3 (remaining gated wins; v1 landed — see Done).** v1
  (2026-06-17) made the gated *indirect-load* operand native (`*p == c` → `rep; lda (zp); cmp; sep`, −4 B).
  Remaining, each gated to stay **no-regression** (a spike showed the *blanket* form regresses register
  +8 B / global +4–12 B operands, where the native compare routes the LHS through `Imag16` + `rep`/`sep`
  that the tight 8-bit `cpx;cmp` avoids): **v2** — computed/`Imag16`-resident LHS (`(a+b) == c`, −3 B):
  extend `isIndirectS16Load` to a computed-LHS operand, after verifying the *other* operand isn't a
  real-register value that would force a spill (measure first). **v3** — abs-operand fold for globals
  (`g1 == g2`): add `LDAbs16`/`CMPAbs16` folding to the EQ native path (mirror `a16abscmp`/`selectSbc16`)
  so a global is read in place, turning the +4/+12 B blanket-regression into a win. Modest gains, worth
  doing (amplified across the toolchain); best sequenced with A16-threading (changes operand residency —
  re-measure then). [native-EQ gated impl plan](docs/plans/2026-06-16-321-native-s16-eq-gated-impl.md) ·
