...
- [ ] **#321 native s16 equality-as-value — v2 (v1 + v3 + `g1 == 0x1234` landed — see Done).**
  v1 (2026-06-17) made the gated *indirect-load* operand native (`*p == c`, −4 B); v3 (2026-06-17) folds
  *both-global* `g1 == g2` to native `lda abs; cmp abs` (−48 B on a chained test); the `g1 == 0x1234`
  follow-up (2026-06-17) folds global-vs-immediate to `lda abs; cmp #imm` (`CmpBrAbsImm16`) and lit up the
  dormant `CmpBrImm16`. Remaining: **v2** — computed/`Imag16`-resident LHS (`(a+b) == c`, −3 B): extend
  the gate to a computed-LHS operand, after verifying the *other* operand isn't a real-register value that
  would force a spill (measure first). Best sequenced with A16-threading (changes operand residency).
