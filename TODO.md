...
- [ ] **#321 native s16 — 16-bit comparison follow-ups** (unsigned ordering, ~~(a) equality `== !=`~~,
  ...
  folds the LHS via `lda abs`). **Tier-1 fuzzer finding F3 (the 8 fuzz XFAILs — the `SelectImm $a16`
  crash) is FIXED** (2026-06-16, an `Ac16`-spill-across-a-call register-allocator bug — NOT this
  comparison/legalizer item; the legalizer-gate guess was tried and reverted. See Done +
  `examples/65816/a16spill.c`). Remaining here: the soft-stack spill path (`expandLDSTStk`) has the same
  `Imag16`-only gap for `Ac16` — pre-existing, unreachable by the nonreentrant corpus, harder (`XBA` for
  the accumulator high byte); it crashes loudly under verify, never miscompiles.
