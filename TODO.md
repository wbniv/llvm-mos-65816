...
- [ ] **`+mos-xy16` miscompile — iterative in-place `memmove`/`memcpy` rewrite over a 16-bit-indexed buffer** (found 2026-06-29 by the #23 L-system demo's differential gate; **confirmed real**, toolchain `c798c31`, not a stale build). A loop that rewrites a `>256`-byte buffer in place (memmove tail-shift + memcpy production, pointers/length from a 16-bit loop index) produces wrong bytes under `+mos-xy16` only (`xy16=0x1CC6` vs host==default==a16 `0x90AA`). Isolated: needs xy16 **AND** 16-bit index (`CAP=200` 8-bit is clean) **AND** the iterative loop (single expansion clean; ping-pong clean). `-verify` clean, so **distinct from the `8c928b8` legalizer-domination fix** (tested on an isolated worktree toolchain with `8c928b8` applied — its `legalindexdom.c -verify` gate is clean there, yet my repro still diverges). **Root cause (hypothesis):** the seed-56 workaround in `MOSLegalizerInfo::tryAbsoluteIndexedAddressing`, `Explicit16 = G_MERGE(trunc(NewOffset,s8), 0)`, keeps only the **low byte** of the index (correct `<256`; drops the high byte of a genuine 16-bit index under xy16). `8c928b8` moved *where* that MERGE is emitted but kept the `trunc`. Fix likely = widen/zero-extend `NewOffset` to 16-bit on the `HasIndex16`/xy16 path instead of `trunc`. Clean repro: `examples/65816/xy16-inplace-memmove-repro.c` (CAP toggle). ([investigation](docs/investigations/2026-06-29-xy16-inplace-memmove-16bit-index-miscompile.md)). **Same function as `8c928b8` — coordinate with the legalizer-indexed-addressing owner. Blocks #23 L-system 5-way-green.**
...
- [ ] **Compiler stress-test demo battery — algorithm+visual SNES demos**
  ...
  - [wip] **#23 L-System Plant** (Round 2) — grows an L-system by **string rewriting** (each generation copies every symbol's production into a grown char buffer with **`memcpy`** (runtime length) + **`strlen`**), then a turtle interprets it into a fractal plant with a **`[`/`]` bracket push/pop stack** — the string-libcall + save/restore-stack corners no other demo runs. Integer fixed-point (Q8.8 + SINCOS) ⇒ bit-exact; bank-0 buffers ⇒ 5-way. Growing fractal plant = the visual proof. ([plan](docs/plans/2026-06-29-23-snes-lsystem-string-rewriting.md))
