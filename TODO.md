...
- 2026-06-21 — [320-inc4-ph2-far-cc] **#320 Inc 4 Phase 2 — far-pointer CALLING CONVENTION: built all 4 ABI variants, measured, shipped (a) Imag32 default-on — the cross-call capability is complete.** A far (addrspace 2) pointer crossing a call (arg or return) used to crash call lowering (`CC_MOS` sized every ptr 16-bit → 24-bit `G_UNMERGE`). Built all four candidate ABIs behind off-by-default `+mos-farcc-*` features — **(a) Imag32** (one 4-byte `RL` quad, `10a5fc0`), **(b) Imag16+bank** (`RS#`+`RC#` custom split via `assignCustomValue`, `741a8c2`), **(c) A:X+Y** (3-way hardware-reg split, `02953e7`), **(d) soft-stack** (one 4-byte RS0-relative slot via `CCAssignToStack` + an `assignValueToAddress` ptrtoint/inttoptr guard, `ebaa515`) — then **measured** with a net-new harness (`examples/65816/farcc_bench.c` + `dev/probe-cycles.lua` + `dev/measure-far-cc.sh`; MAME 0.285 has no Lua `total_cycles()`, so the metric is round-trips completed in a fixed wall of frames, `ceaded6`): **(a) 70 B / 50441 rt — smallest AND fastest**; (b) 86 / 41385, (c) 102 / 43572, (d) 174 / 30626 — every other variant bigger AND slower. **D** (`b2bcdd1`): `MOSSubtarget::farPtrCC()` now defaults to `Imag32`, so a far ptr crosses a call with **no flag** (the #320 Inc 4 ship); b/c/d remain opt-in overrides (the inert `0004` spike); **variant (d) hardware-`,S` recorded-and-dropped** (`SPAdj==0` at `MOSRegisterInfo.cpp:278` + missing 65816 `,S` opcodes; frame-ABI already measured 0/13). Verified: no-flag == winner (byte-identical), corpus 7/7, all 12 far ROMs `0xF3` on MAME + bsnes-jg, the 4 `farcc_*` gates pass (negative controls flipped to default-on/opt-in). Upstream #320 CC evidence note drafted + queued (user-triggered). [plan](docs/plans/2026-06-20-320-far-pointer-cc-build-all-variants.md) · [M+D](docs/plans/2026-06-21-320-far-cc-M-measure-D-land-winner.md) · [variant d](docs/plans/2026-06-21-320-far-cc-variant-d-stack.md).
...
<!-- triaged 2026-06-21: all five are the M+D plan's RESOLVED non-goals + its now-satisfied [verify].
     • "not building variant (d) hardware-`,S`" + "not splitting the winner out of 0004 into 0001" + "not
       absolute cycle counts" -> scope boundaries of the (now-RESOLVED) study: record-and-drop (sub-plan),
       a documented patch-layout decision (0004 stacks on 0002's AnyRegBank line), and a measured constraint
       (MAME 0.285 has no Lua total_cycles()). Nothing open.
     • "not posting upstream" -> the user-triggered "#320 post design note upstream" bullet + the queued
       #320 CC evidence note (upstream-contribution-status row 7). Tracked, user-triggered.
     • [verify] -> SATISFIED: M+D shipped + verified (no-flag==winner, corpus 7/7, 12 far ROMs 0xF3 on both
       emulators); recorded in the M+D plan Status + the Done entry above. Nothing open.
     fp:22834bf4cff4871f fp:791ac5628feb80c1 fp:5c80fc5e2b511bd9 fp:08ef1a18b66d3235 fp:6a89972340f81bd6 -->
