...
- [ ] **#321 native s16 — agreed optimization order (after load-fold).** ~~(2) 16-bit compares/branches~~
  ...
  `>>`/ASHR done — see Done; ~~1-byte `inc a`/`dec a`~~ done — see Done; remaining: variable shifts
  [libcall today, low value], amount ≥8 [byte-relabel already optimal], memory-RMW `inc abs`/`dec abs`
  [next]); ~~(4) indexed/array access~~ (indirect `(zp)` load/
