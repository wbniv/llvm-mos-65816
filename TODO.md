...
- [ ] **#321 native s16 equality-as-value — v2 + the `g1 == 0x1234` follow-up (v1 + v3 landed — see Done).**
  v1 (2026-06-17) made the gated *indirect-load* operand native (`*p == c`, −4 B); v3 (2026-06-17) folds
  *both-global* `g1 == g2` to native `lda abs; cmp abs` (−48 B on a chained test). Remaining: **v2** —
  computed/`Imag16`-resident LHS (`(a+b) == c`, −3 B): extend the gate to a computed-LHS operand, after
  verifying the *other* operand isn't a real-register value that would force a spill (measure first).
  **`g1 == 0x1234` (global vs immediate)** — deferred from v3: the 16-bit constant is byte-split into
  `G_MERGE_VALUES(i8,i8)` before selection, so `m_CmpNZImm16` can't recover it (the existing `CmpBrImm16`
  is dormant for the same reason). Fix: teach the `CmpNZ16` matcher to reconstruct the i16 constant
  through `G_MERGE_VALUES` of two byte constants — lights up `CmpBrImm16` (helps v1's `*p == c`) and
  unblocks a `CmpBrAbsImm16`. Best sequenced with A16-threading (changes operand residency — re-measure).
  [v3 abs-fold plan (done)](docs/plans/2026-06-17-321-native-s16-eq-as-value-v3-abs-fold-globals.md) ·
  [native-EQ gated impl plan](docs/plans/2026-06-16-321-native-s16-eq-gated-impl.md) ·
  [design + spike](docs/plans/2026-06-16-321-native-s16-eq-as-value-cmpsel.md).
