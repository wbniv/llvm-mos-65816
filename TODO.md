...
- [ ] **#321 native s16 — 16-bit comparison follow-ups** (unsigned ordering, ~~(a) equality `== !=`~~,
  ...
  folds the LHS via `lda abs`). **⚠ Tier-1 fuzzer finding F3 (the 8 remaining fuzz XFAILs) — root cause
  CORRECTED 2026-06-16, now a Tier-2 register-allocator item (NOT this comparison/legalizer item):**
  in branchy control flow a compare result consumed as a cross-block i1 VALUE crashes with
  `SelectImm $a16,…`/`$x` (a GPR where a Flag reg is required) → invalid MIR / backend segfault. The
  first-guess legalizer-gate fix (the s16 ORDERING gate at `MOSLegalizerInfo.cpp:1366` lacking the
  EQUALITY gate's all-uses-are-`G_BRCOND_IMM` guard) was **implemented and DISPROVEN** — clean post-isel
  SSA, but the crash is **post-register-allocation** and a clean UGE-as-value already compiles
  native-and-valid, so the gate only pessimizes. F3 is the **A16↔8-bit register-coalescer crash
  resurfacing via spill copies**: an i1 entangles with the 16-bit-accumulator (`ac16`) live range when a
  16-bit accumulator value is forced across a call under branchy/i1 pressure, and
  `MOSInstrInfo::copyPhysRegImpl` (`:743`, the `Anyi1` COPY branch) materializes the spill as
  `SelectImm $a16`. **Fix is Tier-2 register allocation** (stop `Anyi1`↔`ac16` coalescing, or give
  `ac16` a real 16-bit spill); reverted cleanly, XFAIL kept. Repro
  `examples/65816/known/a16-cmp-value-selectimm.c` (seeds 1,7,9,11,22,35,41,44).**
  [F3 plan §Outcome](docs/plans/2026-06-16-321-fix-cmp-value-selectimm.md) ·
...
- 2026-06-16 — [321-fix-cmp-value-selectimm] **F3 root cause CORRECTED: the `SelectImm $a16` crash is a register-allocator bug, NOT the legalizer compare-gate — candidate A disproven, reverted clean, XFAIL kept.** The next-pass hypothesis (s16 ordering native gate lacks the equality gate's all-uses-are-`G_BRCOND_IMM` guard) was implemented (shared `s16CmpResultIsBranchOnly` helper gating UGE + the SLT→ULT rewrite) and **disproven**: it produces clean post-isel SSA but all 8 XFAIL seeds (1,7,9,11,22,35,41,44) still crash identically, and a clean `r=(a>=b)` UGE-as-value already compiles native-and-valid without it — so the gate only pessimizes. Walked the pass pipeline: the crash first appears after `postrapseudos`, emitted by `MOSInstrInfo::copyPhysRegImpl:743` (the `Anyi1` COPY branch) — an i1 entangled with the 16-bit-accumulator (`ac16`) live range when `arr[in_idx & 7]` keeps a 16-bit accumulator value live across the `f0()` call under branchy/i1 pressure; the spill copy materializes as `SelectImm $a16` (invalid). This is the **A16↔8-bit coalescer crash** (ROADMAP §5 Increment 1d) **resurfacing via spill copies** — Tier-2 register-allocator work. Reverted byte-exact (`0002` round-trips, `git status` clean); docs (F3 plan §Outcome, Tier-1 §Findings, ROADMAP §5, repro comment, this item) re-pointed to register allocation. [plan §Outcome](docs/plans/2026-06-16-321-fix-cmp-value-selectimm.md).
