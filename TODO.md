...
- [wip] **#321 Mandelbrot zoom pyramid — TRUE increasing detail as you zoom in.** **Phase 1 DONE+green** (see
  Done, `wt/321-mandel-zoom` `a54041b`): the host bakes a stack of 2×-finer levels, the SNES DMAs the next
  finer level as zoom crosses each 2× threshold (instant, zero on-console fractal math); single-bank proof
  64×64 × 6 levels = 32× deep, dives into the real-axis mini-Mandelbrot, builds default+`+mos-a16`, per-level
  hash + scripted-zoom differential green. **Phase 2 REMAINS (feasibility PROVEN, mechanical):** multi-bank
  LoROM for 128×128 × 8 levels (256×). The crux — DMA sourcing chr from a high bank via the far-symbol
  `bank:addr16` — is proven (spike `docs/plans/spikes/2026-06-25-321-mandel-zoom-phase2-bank-dma-probe.c`);
  remaining = a `snes-zoom` 8-bank platform (mirror `snes-far`), bake multi-bank mode, `snes-checksum.py`
  256 KiB, per-level far-symbol→DMA, larger `dev/mandel-zoom.sh`. De-risked step list in the
  [plan §Phase 2](docs/plans/2026-06-25-321-mandelbrot-zoom-pyramid.md).
- [ ] **#321 / baseline — DEFAULT-8bit matrix-fold-loop MISCOMPILE (cvise → backend fix).** The zoom-pyramid
  differential caught a genuine **default (non-`+mos-a16`) 6502 miscompile**: `zoom_fold`'s `for (i<4) {
  fold((uint8_t)m[i]); fold((uint8_t)((uint16_t)m[i]>>8)); }` folds `0x456E` on the default build vs
  host==`+mos-a16`==`0x456E`→`0xB115` correct; the unrolled form is correct everywhere. Context-sensitive (a
  standalone minimal repro does NOT trigger; `view.h`'s identical `view_fold` loop + corpus/csmith are clean) —
  a narrow regalloc-class baseline defect, INDEPENDENT of #321. Worked around by unrolling (`examples/snes/
  zoom.h`). Next: cvise-reduce from the in-situ trigger (loop vs unroll, default `-Os`) → minimal `.ll` →
  backend fix. Recorded in `zoom.h` + the [plan §Findings](docs/plans/2026-06-25-321-mandelbrot-zoom-pyramid.md).
