...
- [x] **#321 `+mos-a16 -O1/-Os` register-allocation FAILURE on real code (corpus `globals.c`) — FIXED
  2026-06-24, fork patch `0009`** (see Done; the 2026-06-18 "no targeted fix / only the general Phase-3
  rework" conclusion below was REFUTED by a fresh asserts pinpoint: the deadlock's *final blocker* is the
  A-pinned i8 byte index, not `Ac16` over-coalescing — relocating it off the singleton `Ac`={A} to a
  `G_INC`/`G_DEC` chain under `hasAccum16()` dissolves it, DEFAULT byte-identical, net −123 B. The remaining
  s16-pressure core — scavnz / `a16-zp-pressure-overflow` — has no i8 byte index and stays deferred Phase-3).
  Historical record: surfaced by
...
- 2026-06-24 — [321-a16-regpress-fix] **#321 `+mos-a16 -O1/-Os` RA crash `regalloc-out-of-registers` (`globals.c`/`a16regpress.c`) FIXED — fork patch `0009`.** Asserts `-debug-only=regalloc` pinpoint: the strength-reduced i8 array byte index, stepped `i += 2`, selected to A-pinned `ADCImm` (class `Ac`={A}, adc is hardware-A-only) and deadlocked greedy against the 16-bit indexed-load `Ac16`=A:B transit — the counter couldn't recolor off the singleton {A} and the single-instruction INF transit couldn't spill. Fix: `MOSInstructionSelector::selectAddSub` lowers a small-const i8 add/sub (`|Amt|≤2`) to a relocatable `G_INC`/`G_DEC` chain (`selectIncDecMB`→Anyi8=A/X/Y/zp) under `hasAccum16()` instead of the `add Ac,imm→ADCImm` selImpl pattern, so the index coalesces into X (`inx;inx;cpx`) and frees A16. Gated → DEFAULT byte-identical; net **−123 B over 122 c-torture progs (0 worse)**; corpus 7/7, a16 suite 57/0, fuzz 50 0-mismatch, `-verify` clean. `a16regpress.c` + the original `globals.c` compile+run clean; positive gate `dev/run.sh a16regpress` (0x01A7, MAME+bsnes-jg). `KNOWN_ISSUES["regalloc-out-of-registers"]` + its repro row dropped (recurrence hard-FAILS). Refuted the design-workflow synthesis's "the +2 is a 16-bit address IV, not an i8 add" premise empirically (legalized MIR: `G_ADD s8 ,2`). The single-accumulator s16 pressure CORE (scavnz `scavenger-p-not-gpr`, `a16-zp-pressure-overflow`/pr15296) stays deferred Phase-3 — orthogonal (no i8 byte index). [plan](docs/plans/2026-06-24-321-a16-pressure-fix-implementation.md).
- 2026-06-22 — [320-far-tail-calls] **#320 far tail calls DONE — far→far tail folds 5 B→4 B, verified both emulators.** New `TailJML` pseudo (→ `JMP_AbsoluteLong` `$5C`, `R_MOS_ADDR24`) + a far arm in `MOSLateOptimization::tailJMP`: `JSL <direct far global>; RTL → TailJML`, gated `isGlobal && .far_` so near→far (`JSL;RTS`) + the bank-0 thunks (external/non-`.far_` symbols) auto-exclude — conservative (a misclass only misses a win). a16-independent → regenerated into the worktree's `0001` (round-trips `0001..0007`; `MOSLateOptimization.cpp` added to `FAR_FILES`); land to `main` pending. New `dev/run.sh far_tail` (`far_outer` single fold + execution-discriminating two-block `far_pick` → `0xCB`) MAME+bsnes-jg; negative gate in `far_near_call.sh` (thunk tail NOT converted); `+mos-a16` verify clean; corpus 7/7, far suite 12 ROMs, csmith 50 0-mismatch. Design + impl each 3-agent-workflow reviewed (all ship; one test-strengthening + one comment fix applied). Worktree `wt/320-far-tailcall` retained till upstream. [plan](docs/plans/2026-06-22-320-far-tail-calls.md).
