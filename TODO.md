...
- [ ] **#321 native s16 equality-as-value — micro-cases (v1/v3/`g1==0x1234`/v2 all landed — see Done).**
  The four gated EQ-as-value wins are done (v1 indirect `*p==c`; v3 both-global `g1==g2`; `g1==0x1234`
  immediate; v2 computed/`Imag16`-resident `(a+b)==c`). Remaining, low-value and **intentionally 8-bit
  today** (no regression): **(a) `computed == global` mixed** (one operand `Imag16`-resident, one a
  global) — would need a mixed `lda imag; cmp abs` fold; **(b) `x == 0` as a value** (`RHSIsZero`, on the
  `G_CMPZ` path); **(c) register/param operands** (deliberately 8-bit — the native form spills). Best
  revisited with A16-threading (ROADMAP step 5), which changes operand residency — re-measure then.
  [v2 plan (done)](docs/plans/2026-06-17-321-native-s16-eq-v2-computed-imag16-lhs.md) ·
  [eq-imm const-merge plan (done)](docs/plans/2026-06-17-321-native-s16-eq-imm-constant-through-merge.md) ·
...
- [ ] **#321 A16-threading — keep the running s16 value live in the accumulator across ops** (item (5)
  above; the ROADMAP-step-5 "biggest win", de-risked now the Tier-1 corpus exists). **Phases 0–1 DONE
